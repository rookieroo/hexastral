...
/**
 * Birth-info handlers (`PUT /api/user/:userId/birth-info`, `PUT
 * /api/portfolio/birth-info`) must invalidate cached charts and rebuild the
 * natal skeleton so downstream surfaces (public web profile, signal/today,
 * report) see fresh data. Wrap in `executionCtx.waitUntil(...)` at the call
 * site so the HTTP response isn't blocked. Errors are swallowed — failure
 * falls through to lazy rebuild on next chart read.
 */
export async function rebuildUserCharts(
  db: AppDb,
  env: CloudflareBindings,
  userId: string
): Promise<void> {
  try {
    // Wipe `report_chapters` too: the chapter fetch route 403s free users
    // on chart drift (chartHash mismatch vs cached row). After a birth-info
    // edit the drift is intentional — clearing stale rows lets `generateChapter`
    // rebuild on demand instead of dead-ending.
    await Promise.all([
    ...
    await buildChartSkeleton(db, env, {
      userId,
    ...
    console.error('[chart-skeleton] rebuildUserCharts failed', userId, err)
  }
}

...
async function maybeRelocalizeReading(
  ...
  lc: string | undefined,
  userId: string,
  /** Explicit relocalize (POST /:id/relocalize) bypasses the freeze; the auto GET path
   *  passes false so a locked mirror is never re-generated by a device-locale toggle. */
  force = false
) {
  if (!lc) return null
  ...
  try {
    const parsed = JSON.parse(row.interpretation) as Record<string, unknown>
    storedLang = (parsed.language as string) ?? null
    localeLocked = parsed.localeLocked === 'true'
  } catch {
    return null
  }
  // Freeze (anti-abuse, ADR-0027): a reader's mirror personalizes to their locale ONCE,
  // then locks — a later device-locale change no longer auto-regenerates. Only the
  // metered relocalize (force) re-generates after that.
  if (localeLocked && !force) return null
  ...
  try {
    ...
    interp.language = lc
    interp.localeLocked = 'true' // ADR-0027: lock after this personalization
    const updated = {
      score: a.result.compatibility.score as number,
      grade: a.result.compatibility.grade as string,
...
// ── POST /:id/relocalize — explicit, metered re-read in another language (ADR-0027) ──
//
// Per-reader locale is freeze-after-first (maybeRelocalizeReading locks the mirror once
// personalized). This is the ONLY path that re-generates a frozen mirror, and it spends
// the `reroll` allowance — the bond sibling of the solo chapter reroll. Pro-gated.
const relocalizeSchema = z.object({ lc: z.string().min(1).max(16) })

bondRoutes.post('/:id/relocalize', zValidator('json', relocalizeSchema), async (c) => {
  const bondId = c.req.param('id')
  const userId = requireUserId(c)
  const db = c.get('db')
  const { lc } = c.req.valid('json')
  ...
  const bond = await db
    .select({ id: userBonds.id, hehunReadingId: userBonds.hehunReadingId })
    .from(userBonds)
    .where(and(eq(userBonds.id, bondId), eq(userBonds.ownerId, userId)))
    .get()
  ...
  const row = await db
    ...
    .where(eq(pairReadings.id, bond.hehunReadingId))
    .get()
  if (!row) return jsonErr(c, 404, ApiErrorCode.not_found, 'Reading not found')
  ...
  // Metered: spend a `reroll` (the bond sibling of the solo chapter reroll).
  ...
  const updated = await maybeRelocalizeReading(
    c.env.SVC_ASTRO,
    db,
    bond.hehunReadingId,
    row,

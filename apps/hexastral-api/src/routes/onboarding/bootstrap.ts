 ...
 * (CF Workers AI standard tier, REVEAL_MODEL).
 *
...
export const onboardingBootstrapRoutes = new Hono<AppEnv>().post(
  ...
  async (c) => {
    ...
    if (!user.birthSolarDate || user.birthTimeIndex == null || !user.birthGender) {
      return jsonErr(c, 400, ApiErrorCode.missing_required, 'Missing birth info', {
        step: 'validate',
      })
    }
    ...
    try {
      ...
      return jsonErr(c, 502, ApiErrorCode.upstream_unavailable, 'Chart skeleton failed', {
        step: 'chart',
      })
    }
    ...
    if (existingSignal) {
      ...
      try {
        ...
        return jsonErr(c, 502, ApiErrorCode.generation_failed, 'Signal generation failed', {
          step: 'signal',
          traits: skeleton,
        })
      }

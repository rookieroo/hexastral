...
import { dailyActivity, divinations, singlePurchases, users } from '../../db/schema'
...
export const divinationRoutes = new Hono<AppEnv>()
  .post('/', async (c) => {
    ...
    // ── 访问鉴权（Pro 配额 or 单次购买）──────────────────────────────────────
    const access = await checkReadingAccess(db, input.userId, 'cast')
    if (!access.granted) {
      return c.json(
  ...
  .get('/quota/:userId', async (c) => {
    ...
    const isPremium = await userHasAnySubscription(db, userId)

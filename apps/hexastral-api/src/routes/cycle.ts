/**
 * GET /api/cycle/* — Cycle (黄历) satellite engine. 100% deterministic, no LLM.
 *
...
cycleRoutes.get('/year-overview', (c) => {
...
type CycleEvent = (typeof EVENTS)[number]

/** event → 黄历 宜忌 verbs that decide its fitness, matched against goodFor / avoid. */
const EVENT_VERBS: Record<CycleEvent, readonly string[]> = {
  wedding: ['嫁娶'],
  business: ['开市', '交易', '纳财'],
  signing: ['立券', '交易'],
  move: ['移徙'],
...
const EVENT_BOOSTS: Partial<Record<CycleEvent, EventBoosts>> = {
  // 嫁娶 — 成日宜婚, 定日宜定盟, 破日大忌.
  wedding: {
...
function runSearch(event: CycleEvent, fromStr: string, toStr: string, specialized: boolean) {
...
// ── POST /explain — 深度解读 (C.4): the ONLY LLM in Cycle ─────
//
...
cycleRoutes.post('/explain', async (c) => {
  ...
  const cacheKey = `cycle:explain:${body.date}:${body.field}:${body.locale}:${bucket}`
  ...
  if (guard.decision === 'allow_llm') {
    try {
      ...
      console.error('[cycle.explain] svc-astro failed', err)
    }

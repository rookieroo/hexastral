...
export interface CloudflareBindings {
  ...
  /** Service binding → hexastral-svc-feng (Mapbox proxy + annotate + 3-stage AI pipeline) */
  SVC_FENG: Fetcher
   ...
   * sweep (ADR-0013 §5b). Distinct from the webhook bearer secret above. */
  ...
  /**
   * Google Sign-In — comma-separated list of accepted OAuth client IDs (the
   * `aud` claim on Google ID tokens, e.g. "<ios>.apps.googleusercontent.com,
   * <web>.apps.googleusercontent.com"). Portfolio Google auth fails closed
   * (503) when unset, so an unconfigured deploy never trusts a token.
   */

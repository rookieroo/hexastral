import AsyncStorage from '@react-native-async-storage/async-storage'
import { getLocales } from 'expo-localization'
import { useEffect, useState } from 'react'

const LOCALE_STORAGE_KEY = 'satellite_locale'

export type UiLocale = 'en' | 'zh' | 'zh-Hant' | 'ja' | 'ko'

export const SUPPORTED_UI_LOCALES: readonly UiLocale[] = ['en', 'zh', 'zh-Hant', 'ja', 'ko']

// DEV-only locale override (Settings · DEV) — preview any shipped locale without changing
// device language. Resets on reload; production always follows device / saved pref.
let devLocaleOverride: UiLocale | null = null

export function getCoincastDevLocale(): UiLocale | null {
  return __DEV__ ? devLocaleOverride : null
}

export function setCoincastDevLocale(next: UiLocale | null): void {
  if (__DEV__) devLocaleOverride = next
}

/** Resolve UI locale: DEV override → saved pref → device. */
export function resolveUiLocale(saved?: string | null): UiLocale {
  const override = getCoincastDevLocale()
  if (override) return override
  if (saved) return normalizeLocale(saved)
  const fallback = getLocales()[0]?.languageTag ?? 'en'
  return normalizeLocale(fallback)
}

/** All UI strings for coin-cast-app (en + zh + zh-Hant + ja + ko). */
const EN = {
  homeShake: 'Shake',
  homeHoldShake: 'Hold to shake',
  homeReleaseOpen: 'Release to open',
  homeCast: 'Cast',
  homeCastingButton: 'Casting…',
  homeInputA11y: 'Your question for this reading',
  homeQuestionPrompt: 'Tap to ask your question',
  questionSheetTitle: 'Your question',
  questionSheetCta: 'Begin casting',
  homeAbortA11y: 'Stop this casting',
  homeProgressA11y: '{n} of six lines cast',
  homeYaoCoinBarLegend:
    'Three small marks: coin faces. Line drawing follows classical I Ching rules (7/8 are not “majority yin/yang”).',
  homeYaoRowA11y: 'Line {n}: {faces}. Throw total {total}.',
  homeYaoPosition: 'Pos',
  homeYaoThrow: 'Throw',
  coinFaceA11yYin: 'Yin face',
  coinFaceA11yYang: 'Yang face',
  homeGlFallback:
    '3D preview needs a WebGL dev build. Tosses still work; lines show after each throw.',
  homeDevShakeHint:
    'Dev builds: use the Shake button (system may reserve device shake for the dev menu).',
  settingsTitle: 'Settings',
  settingsMotionLabel: 'Shake to cast',
  settingsMotionHint: 'When on, a firm shake counts as one toss of the three coins.',
  settingsMotionOn: 'On',
  settingsMotionOff: 'Off',
  settingsSensorySection: 'Casting & feel',
  settingsHapticsLabel: 'Vibration feedback',
  settingsHapticsHint: 'Haptic pulses when coins land and when each line completes.',
  settingsCoinSkinLabel: 'Coin appearance',
  settingsCoinSkinHint:
    'Default is the Yaul logo coin — round with a square hole, matching the app icon.',
  settingsCoinSkinDefault: 'Logo default',
  settingsCoinSkinCustomActive: 'Custom coin face',
  settingsCoinSkinUpload: 'Upload coin face',
  settingsCoinSkinUploadHint: 'Your image is scaled to fit without cropping.',
  settingsCoinSkinUploading: 'Uploading…',
  settingsCoinSkinUploadFailedTitle: 'Upload failed',
  settingsCoinSkinUploadFailedBody:
    'Could not save the coin face. Check photo access and try a JPEG or PNG.',
  settingsCoinSkinPermissionBody: 'Photo access is required to choose a custom coin face.',
  settingsCoinSkinNativeBody:
    'This build does not include the photo or image-processing module. Install a newly rebuilt app and retry.',
  settingsCoinSkinFormatBody:
    'This image could not be decoded. Choose a valid JPEG, PNG, or HEIC image.',
  settingsCoinSkinStorageBody: 'The normalized coin image could not be saved to device storage.',
  settingsOpenSystemSettings: 'Open Settings',
  settingsCoinSkinReset: 'Use logo default',
  settingsAccountSection: 'Account',
  settingsAccountAppleKind: 'Apple account',
  settingsAccountSignedIn: 'Signed in',
  settingsDeleteAccount: 'Delete account & data',
  settingsDeleteAccountHint:
    'Permanently removes your account and reading history. Cannot be undone.',
  settingsDeleteAccountTitle: 'Delete account?',
  settingsDeleteAccountBody:
    'This permanently deletes your HexAstral account and Yaul reading history from our servers. This cannot be undone.',
  settingsDeleteAccountContinue: 'Delete',
  settingsDeleteAccountFailed: 'Could not delete account. Please try again.',
  settingsDeleteAccountCancel: 'Cancel',
  sceneLoading: 'Loading scene…',
  settingsMemoryLabel: 'Personalized reading memory',
  settingsMemoryHint:
    'When on, short summaries of your past AI readings may be retrieved to enrich new AI casts. Classical casts are not indexed. Stored on HexAstral servers; not sold. You can turn this off anytime.',
  settingsMemoryOn: 'On',
  settingsMemoryOff: 'Off',
  settingsMemoryGuestHint: 'Sign in with Apple to enable memory across devices.',
  settingsMemorySaving: 'Saving…',
  beforeCastTitle: 'Before you cast',
  beforeCastC1Title: 'One question, sincere intent',
  beforeCastC1Body:
    'Ask one clear question this round and keep it after the lines start. Reflect: real doubt, honesty, no harm to others.',
  beforeCastC2Title: 'Settle',
  beforeCastC2Body: 'Breathe slowly with the pulse—when you feel centered, go on.',
  beforeCastC3Title: 'Ready',
  beforeCastC3Body: 'Tap below to start casting.',
  beforeCastNext: 'Next',
  beforeCastReady: 'I am ready',
  breathingOverlayText: 'Breathe… stay with your question.',
  alertDuplicateTitle: 'Same question recently',
  alertDuplicateMsg:
    'You asked this same question within the last day. Continue only if you truly mean to revisit it.',
  alertDuplicateContinue: 'Continue anyway',
  alertDuplicateCancel: 'Cancel',
  alertRefusedTitle: 'Reading not offered',
  alertRefusedTradition: 'The I Ching asks that we approach with a sincere heart.',
  alertRefusedWarn:
    'Repeated insincere or trivial questions may result in a brief pause from Yaul.',
  alertBannedTitle: 'Yaul paused',
  alertBannedMsg: 'This pause lifts after {time}. Please return with a sincere question.',
  alertFirstTitle: 'Please read first',
  alertFirstMsg: 'Open the short guide once before your first cast.',
  alertFirstOpen: 'Open',
  alertCooldownTitle: 'Please wait',
  alertCooldownMsg: 'Wait about {m} more minute(s) before a new reading.',
  alertAbortTitle: 'Stop casting?',
  alertAbortMsg:
    'Stopping mid-way is discouraged. You can reset and begin again with a clear mind.',
  alertContinue: 'Continue',
  alertAbortConfirm: 'Stop anyway',
  meHistoryTitle: 'Reading history',
  meEmpty: 'No cast history yet.',
  meViewAllHistory: 'View all history',
  meSettings: 'Casting settings',
  meUpgrade: 'Upgrade to Pro',
  restorePurchases: 'Restore purchases',
  mePrivacy: 'Privacy policy',
  meTerms: 'Terms of service',
  meSignOut: 'Sign out',
  meSignInSectionTitle: 'Sign in',
  meSignInHint: 'Use Apple to sync reading history across your devices.',
  meAppleContinue: 'Continue with Apple',
  meApplePreparing: 'Preparing sign-in…',
  meAppleUnavailable:
    'Apple sign-in needs a development or App Store build (not available in Expo Go).',
  meSignInIosOnly: 'Apple sign-in is available on iOS to sync history.',
  meSealByline: 'by HexAstral',
  mePromoBody: 'Unlock your complete fate chart in the flagship app.',
  mePromoCta: 'Open HexAstral',
  stackMe: 'Me',
  stackHistory: 'History',
  stackResult: 'Result',
  stackDetail: 'Interpretation',
  stackCast: 'Cast',
  stackPaywall: 'Yaul Pro',
  stackSettings: 'Settings',
  navBack: 'Back',
  profileSignOutConfirm:
    'Sign out on this device? Reading history stays on the server until you delete your account.',
  stackBeforeCast: 'Before you cast',
  resultOpenDetail: 'Open full interpretation',
  resultReadingId: 'Reading ID: {id}',
  resultHexagramTitle: 'Hexagram {num} {name}',
  resultClassicalBadge: 'Classical reading',
  resultUnlockAiCta: 'Unlock AI deep read',
  upgradeAiCta: 'Upgrade this reading to AI',
  upgradeAiLoading: 'Upgrading…',
  upgradeAiAlreadyDone: 'Already AI',
  historyTierClassical: 'Classical',
  historyTierAi: 'AI',
  detailTitle: 'Detailed reading',
  detailFallbackAdvice: 'Keep your next move simple and observable. Let action answer anxiety.',
  detailClassicalAdviceHint:
    'This reading cites the I Ching corpus and na-jia structure. Purchase a cast pack or Pro for AI commentary on your next cast.',
  detailFallbackSummary: 'Hold center and move.',
  detailClassicalTitle: 'Classical reference',
  detailNaJiaTitle: 'Na-jia hexagram',
  detailJudgment: 'Judgment',
  detailImage: 'Image',
  detailLines: 'Line statements',
  detailKeywords: 'Themes',
  resultFallbackInterpretation:
    'Your cast result is ready. Continue to full detail for chapter-style guidance.',
  paywallUnlock: 'AI commentary & Pro',
  paywallRestore: 'Restore purchases',
  paywallRestoreRow: 'restore',
  paywallPlanCastPack1: '1 AI cast',
  paywallPlanCastPack5: '5 AI casts',
  paywallPlanCastPack10: '10 AI casts',
  paywallPlanMonthly: 'Monthly Pro',
  paywallPlanAnnual: 'Annual Pro',
  paywallLegalDisclaimer:
    'Based on classical I Ching Liu Yao — for entertainment, cultural study, and personal reflection only. Not medical, legal, financial, or life advice.',
  resultLegalDisclaimer:
    'This reading is generated for cultural exploration and reflection — not a prediction or professional advice.',
  detailLegalDisclaimer:
    'Classical corpus and AI commentary are for study and reflection only — not fortune-telling or decision advice.',
  devtoolsTitle: 'Developer tools',
  devtoolsReplayOnboarding: 'Reset onboarding',
  devtoolsClearRitual: 'Clear ritual & cooldown prefs',
  devtoolsPortfolioUser: 'Portfolio user',
  devtoolsShowEnv: 'Show build info',
  devtoolsDone: 'Saved locally.',
  devtoolsOk: 'OK',
  castTitle: 'Your question',
  castPlaceholder: 'What should I watch for this week?',
  castSubmit: 'Cast lines',
  castBack: 'Back',
  castError: 'Failed to generate reading. Please retry.',
  homeError: 'Failed to generate reading. Please retry.',
  alertQuotaTitle: 'Reading limit',
  alertQuotaMsg:
    'Free classical casts for this month are used up. Buy an AI cast pack or Pro for personalized commentary.',
  alertQuotaGuestDailyMsg:
    'You have used today’s three guest classical casts (UTC). Sign in with Apple to continue with your linked quota, or buy AI cast packs.',
  alertQuotaSignIn: 'Sign in',
  alertQuotaUpgrade: 'View plans',
  motionUnavailableTitle: 'Sensing unavailable',
  motionUnavailableMessage:
    'This device cannot run motion-driven coin physics. Retry sensing or explicitly use a digital assisted cast.',
  motionPermissionMessage:
    'Motion access was not granted. Enable Motion & Fitness access in Settings, then retry.',
  motionInsufficientMessage:
    'This motion session was incomplete or too gentle to form a motion-caused line. Hold, shake through a full rhythm, then release and retry.',
  motionInterruptedMessage:
    'Sensing was interrupted before this line was complete. Keep Yaul active and retry.',
  motionPhysicsTimeoutMessage:
    'The physical coins did not reach a readable resting state. Shake again from a closed palm.',
  motionRetry: 'Shake again',
  motionRetryPreferredTitle: 'Shake again',
  motionRetryPreferredHint:
    'Digital assistance stays hidden until several consecutive failures — prefer another sincere cast.',
  shakeHoldLonger: 'Keep the palm closed a little longer, with real motion, then release to open.',
  lineSeenTitle: 'Read the fallen coins',
  lineSeenMessage:
    'Leave them as they landed, or neat them into a row once you have read the faces.',
  lineSeenAsIs: 'Leave as fallen',
  lineSeenNeat: 'Neat row',
  digitalAssistAction: 'Digital assisted cast',
  digitalAssistTitle: 'Use digital assisted cast?',
  digitalAssistMessage:
    'This line will use secure digital randomness, not your movement. Its source will be recorded as digital assistance.',
  digitalAssistConfirm: 'Use digital assistance',
  waYingTitle: 'Outer sign — this hexagram is void',
  waYingMessage:
    'A coin settled on its edge: too rare to read as yin or yang. Classical Liu Yao treats this as an outer sign (外应) — the whole hexagram is void, including lines already cast. Do not soft-fail into the next line. Settle your mind, then re-cast this reading from the first line (初爻). It often hints the matter is unsettled, finely balanced, or not yet ready for a fixed verdict.',
  waYingAck: 'Re-cast from the first line',
  deskBenLabel: 'Original hexagram',
  deskBianLabel: 'Changed hexagram',
  deskHuLabel: 'Mutual hexagram',
  deskPalace: 'Palace {palace}',
  deskShiYing: 'World {shi} · Response {ying}',
  deskTableTitle: 'Na-jia lines',
  deskColYao: 'Line',
  deskColGanZhi: 'Stem-branch',
  deskColLiuQin: 'Relative',
  deskColMark: 'Mark',
  deskMarkShi: 'World',
  deskMarkYing: 'Response',
  deskMarkDong: 'Moving',
  deskMarkKong: 'Empty',
  deskNoBian: 'No changing lines — no derived hexagram',
  onboardKicker: 'HEXASTRAL SATELLITE',
  onboardSubtitle:
    'Toss three coins, build six lines. Yaul is an I Ching Liu Yao study journal — ask one sincere question, cast a hexagram, and read classical or AI commentary for reflection.',
  onboardGetStarted: 'Get Started',
  onboardContinue: 'Continue',
  onboardBack: 'Back',
} as const

const ZH: Record<keyof typeof EN, string> = {
  homeShake: '摇卦',
  homeHoldShake: '按住摇卦',
  homeReleaseOpen: '松手开掌',
  homeCast: '成卦',
  homeCastingButton: '解读中…',
  homeInputA11y: '本次占问的问题',
  homeQuestionPrompt: '点按，写下你的问题',
  questionSheetTitle: '你的问题',
  questionSheetCta: '开始摇卦',
  homeAbortA11y: '中断本次摇卦',
  homeProgressA11y: '已完成 {n} 爻',
  homeYaoCoinBarLegend: '三划为钱面；爻条为周易画法，7/8 不按钱面多数定阴阳。',
  homeYaoRowA11y: '第{n}爻：{faces}，掷得{total}。',
  homeYaoPosition: '爻位',
  homeYaoThrow: '掷得',
  coinFaceA11yYin: '阴面',
  coinFaceA11yYang: '阳面',
  homeGlFallback: '3D 预览需含 WebGL 的构建；摇卦与爻线照常。',
  homeDevShakeHint: '开发版请用下方「摇卦」；系统可能占用体感摇一摇。',
  settingsTitle: '设置',
  settingsMotionLabel: '摇一摇起爻',
  settingsMotionHint: '开启后，用力摇晃手机可代替一次三钱落地。',
  settingsMotionOn: '开启',
  settingsMotionOff: '关闭',
  settingsSensorySection: '起卦与体感',
  settingsHapticsLabel: '震感反馈',
  settingsHapticsHint: '铜钱落地与每爻成卦时的触觉振动。',
  settingsCoinSkinLabel: '铜钱外观',
  settingsCoinSkinHint: '默认使用 Yaul Logo 方孔钱，与应用图标一致。',
  settingsCoinSkinDefault: 'Logo 默认',
  settingsCoinSkinCustomActive: '自定义钱面',
  settingsCoinSkinUpload: '上传钱面图片',
  settingsCoinSkinUploadHint: '图片会完整适配显示，不会裁剪。',
  settingsCoinSkinUploading: '上传中…',
  settingsCoinSkinUploadFailedTitle: '上传失败',
  settingsCoinSkinUploadFailedBody: '无法保存钱面图片。请确认相册权限，并尽量使用 JPEG 或 PNG。',
  settingsCoinSkinPermissionBody: '需要相册权限才能选择自定义钱面。',
  settingsCoinSkinNativeBody: '当前安装包缺少相册或图片处理原生模块。请安装重新构建的应用后再试。',
  settingsCoinSkinFormatBody: '无法解析这张图片。请选择有效的 JPEG、PNG 或 HEIC 图片。',
  settingsCoinSkinStorageBody: '规范化后的钱面图片无法保存到设备存储。',
  settingsOpenSystemSettings: '打开系统设置',
  settingsCoinSkinReset: '恢复 Logo 默认',
  settingsAccountSection: '账号',
  settingsAccountAppleKind: 'Apple 账号',
  settingsAccountSignedIn: '已登录',
  settingsDeleteAccount: '删除账号与数据',
  settingsDeleteAccountHint: '永久删除账号与解读历史，无法恢复。',
  settingsDeleteAccountTitle: '删除账号？',
  settingsDeleteAccountBody: '将永久删除您的 HexAstral 账号及 Yaul 解读历史。此操作无法撤销。',
  settingsDeleteAccountContinue: '删除',
  settingsDeleteAccountFailed: '删除失败，请稍后重试。',
  settingsDeleteAccountCancel: '取消',
  sceneLoading: '场景加载中…',
  settingsMemoryLabel: '个性化解读记忆',
  settingsMemoryHint:
    '开启后，仅 AI 深读记录可能被检索以丰富新的 AI 解读；经典（免费）起卦不会写入记忆。数据存于 HexAstral 服务器，不出售；可随时关闭。',
  settingsMemoryOn: '开启',
  settingsMemoryOff: '关闭',
  settingsMemoryGuestHint: '使用 Apple 登录后可在多设备间开启记忆。',
  settingsMemorySaving: '保存中…',
  beforeCastTitle: '占前必读',
  beforeCastC1Title: '一事一问，诚意',
  beforeCastC1Body: '本轮只问一件事，起爻后勿改问。因疑而问、以诚相对、不为害人之事。',
  beforeCastC2Title: '静心',
  beforeCastC2Body: '随光点放慢呼吸，觉得稳了再下一步。',
  beforeCastC3Title: '开始',
  beforeCastC3Body: '点下方进入摇卦。',
  beforeCastNext: '下一步',
  beforeCastReady: '我已准备好',
  breathingOverlayText: '调匀呼吸……心念你的问题。',
  alertDuplicateTitle: '近期问过同一题',
  alertDuplicateMsg: '你在过去一天内问过这个完全相同的问题。若确有必要再问，请再确认心意。',
  alertDuplicateContinue: '仍要继续',
  alertDuplicateCancel: '取消',
  alertRefusedTitle: '本次不予解读',
  alertRefusedTradition: '易经传统希望我们带着诚意前来。',
  alertRefusedWarn: '若多次以轻佻或无关问题反复占问，系统可能会短暂暂停 Yaul。',
  alertBannedTitle: 'Yaul 已暂停',
  alertBannedMsg: '暂停将在 {time} 后解除。请稍后再来，并以诚意发问。',
  alertFirstTitle: '请先阅读',
  alertFirstMsg: '首次占问前，请先打开简短说明。',
  alertFirstOpen: '打开',
  alertCooldownTitle: '请稍候',
  alertCooldownMsg: '距下次可占问还需约 {m} 分钟。',
  alertAbortTitle: '确定中断？',
  alertAbortMsg: '中途停止并不理想。你可选择继续，或清空后静心再起。',
  alertContinue: '继续',
  alertAbortConfirm: '仍要中断',
  meHistoryTitle: '解读历史',
  meEmpty: '暂无历史记录',
  meViewAllHistory: '查看全部历史',
  meSettings: '起卦与体感',
  meUpgrade: '升级专业版',
  restorePurchases: '恢复购买',
  mePrivacy: '隐私政策',
  meTerms: '使用条款',
  meSignOut: '退出登录',
  meSignInSectionTitle: '登录',
  meSignInHint: '使用 Apple 登录可在设备间同步解读历史。',
  meAppleContinue: '通过 Apple 继续',
  meApplePreparing: '正在准备登录…',
  meAppleUnavailable: 'Apple 登录需开发版或 App Store 版（Expo Go 不可用）。',
  meSignInIosOnly: '在 iOS 上可通过 Apple 登录同步历史。',
  meSealByline: 'HexAstral 出品',
  mePromoBody: '前往主应用解锁完整命盘与深度内容。',
  mePromoCta: '打开 HexAstral',
  stackMe: '我的',
  stackHistory: '历史',
  stackResult: '结果',
  stackDetail: '详解',
  stackCast: '起卦',
  stackPaywall: 'Yaul Pro',
  stackSettings: '设置',
  navBack: '返回',
  profileSignOutConfirm: '在此设备上退出登录？账号与历史记录仍保留在服务器，除非您删除账号。',
  stackBeforeCast: '占前必读',
  resultOpenDetail: '查看完整解读',
  resultReadingId: '记录编号：{id}',
  resultHexagramTitle: '第{num}卦 {name}',
  resultClassicalBadge: '经典解读',
  resultUnlockAiCta: '解锁 AI 深读',
  upgradeAiCta: '将此卦升级为 AI 深读',
  upgradeAiLoading: '升级中…',
  upgradeAiAlreadyDone: '已是 AI 解读',
  historyTierClassical: '经典',
  historyTierAi: 'AI',
  detailTitle: '详细解读',
  detailFallbackAdvice: '把下一步做得简单、可观察。用行动回应焦虑。',
  detailClassicalAdviceHint:
    '本次为卦辞、爻辞与纳甲结构的经典层解读。购买释读包或 Pro 可在下一次起卦获得 AI 解读。',
  detailFallbackSummary: '守中而行。',
  detailClassicalTitle: '原文参考',
  detailNaJiaTitle: '纳甲装卦',
  detailJudgment: '卦辞',
  detailImage: '大象',
  detailLines: '爻辞',
  detailKeywords: '主题',
  resultFallbackInterpretation: '卦象已就绪，可进入全文解读获取更多层次说明。',
  paywallUnlock: 'AI 深读与 Pro',
  paywallRestore: '恢复购买',
  paywallRestoreRow: '恢复',
  paywallPlanCastPack1: '1 次 AI 释读',
  paywallPlanCastPack5: '5 次 AI 释读',
  paywallPlanCastPack10: '10 次 AI 释读',
  paywallPlanMonthly: '月度 Pro',
  paywallPlanAnnual: '年度 Pro',
  paywallLegalDisclaimer:
    '基于经典六爻与《周易》文化——仅供娱乐、文化研习与个人省思，不构成医疗、法律、财务或人生决策建议。',
  resultLegalDisclaimer: '本卦象解读仅供文化探索与个人省思，非预测或专业建议。',
  detailLegalDisclaimer: '经典文献与 AI 释读仅供研习与省思，非算命或决策建议。',
  devtoolsTitle: '开发者工具',
  devtoolsReplayOnboarding: '重置引导',
  devtoolsClearRitual: '清空占前确认与冷却等本地状态',
  devtoolsPortfolioUser: 'Portfolio 用户',
  devtoolsShowEnv: '查看构建信息',
  devtoolsDone: '已保存。',
  devtoolsOk: '好',
  castTitle: '你的问题',
  castPlaceholder: '这周我最该留意什么？',
  castSubmit: '起卦',
  castBack: '返回',
  castError: '生成解读失败，请重试。',
  homeError: '生成解读失败，请重试。',
  alertQuotaTitle: '解读次数',
  alertQuotaMsg: '本月免费经典起卦次数已用完。购买 AI 释读包或 Pro 可获得个性化解读。',
  alertQuotaGuestDailyMsg:
    '今日（UTC）三次免登录经典起卦已用完。请使用 Apple 登录以继续使用账号额度，或购买 AI 释读包。',
  alertQuotaSignIn: '去登录',
  alertQuotaUpgrade: '查看方案',
  motionUnavailableTitle: '感应不可用',
  motionUnavailableMessage: '当前设备无法运行运动驱动的铜钱物理。可重试感应，或明确选择数字代摇。',
  motionPermissionMessage: '尚未获得运动传感器权限。请在系统设置中开启“运动与健身”权限后重试。',
  motionInsufficientMessage:
    '本次运动序列不完整或动作过轻，无法形成运动因果的一爻。请合掌再摇一轮，松手开掌后重试。',
  motionInterruptedMessage: '本爻感应尚未完成便被中断。请保持 Yaul 在前台并重试。',
  motionPhysicsTimeoutMessage: '铜钱未在时限内形成可判读的稳定落面。请合掌再摇。',
  motionRetry: '再摇',
  motionRetryPreferredTitle: '请再摇一次',
  motionRetryPreferredHint: '连续失败数次后才会出现数字代摇；成功路径永不注入代摇。',
  shakeHoldLonger: '请再合掌摇一会儿、带上真实晃动，再松手开掌。',
  lineSeenTitle: '看清落子',
  lineSeenMessage: '可先按落子原样读字背；看清后再可选整齐成排。',
  lineSeenAsIs: '保持落形',
  lineSeenNeat: '整齐排列',
  digitalAssistAction: '数字代摇',
  digitalAssistTitle: '使用数字代摇？',
  digitalAssistMessage: '这一爻将使用安全数字随机，而不是你的运动；来源会明确记录为“数字代摇”。',
  digitalAssistConfirm: '确认数字代摇',
  waYingTitle: '外应 · 本卦不作数',
  waYingMessage:
    '有铜钱立于桌缘、难以定阴阳。按传统六爻以外应论之——本卦整卦作废（含已摇出的爻），不可软失败进入下一爻。请静心后从初爻重新摇齐六爻。此象多示所问悬而未决、变数未落，不必强判吉凶。',
  waYingAck: '从初爻重摇',
  deskBenLabel: '本卦',
  deskBianLabel: '变卦',
  deskHuLabel: '互卦',
  deskPalace: '{palace}宫',
  deskShiYing: '世 {shi} · 应 {ying}',
  deskTableTitle: '装卦表',
  deskColYao: '爻',
  deskColGanZhi: '纳甲',
  deskColLiuQin: '六亲',
  deskColMark: '标记',
  deskMarkShi: '世',
  deskMarkYing: '应',
  deskMarkDong: '动',
  deskMarkKong: '空',
  deskNoBian: '无动爻 · 无变卦',
  onboardKicker: 'HEXASTRAL SATELLITE',
  onboardSubtitle:
    '三枚铜钱，六爻成卦。Yaul 是易经六爻研习工具——诚心问一事，掷钱起一卦，阅读经典或 AI 释读以供省思。',
  onboardGetStarted: '开始',
  onboardContinue: '继续',
  onboardBack: '返回',
}

const ZH_HANT: Record<keyof typeof EN, string> = {
  homeShake: '搖卦',
  homeHoldShake: '按住搖卦',
  homeReleaseOpen: '鬆手開掌',
  homeCast: '成卦',
  homeCastingButton: '解讀中…',
  homeInputA11y: '本次占問的問題',
  homeQuestionPrompt: '點按，寫下你的問題',
  questionSheetTitle: '你的問題',
  questionSheetCta: '開始搖卦',
  homeAbortA11y: '中斷本次搖卦',
  homeProgressA11y: '已完成 {n} 爻',
  homeYaoCoinBarLegend: '三劃為錢面；爻條為周易畫法，7/8 不按錢面多數定陰陽。',
  homeYaoRowA11y: '第{n}爻：{faces}，擲得{total}。',
  homeYaoPosition: '爻位',
  homeYaoThrow: '擲得',
  coinFaceA11yYin: '陰面',
  coinFaceA11yYang: '陽面',
  homeGlFallback: '3D 預覽需含 WebGL 的建置；搖卦與爻線照常。',
  homeDevShakeHint: '開發版請用下方「搖卦」；系統可能占用體感搖一搖。',
  settingsTitle: '設定',
  settingsMotionLabel: '搖一搖起爻',
  settingsMotionHint: '開啟後，用力搖晃手機可代替一次三錢落地。',
  settingsMotionOn: '開啟',
  settingsMotionOff: '關閉',
  settingsSensorySection: '起卦與體感',
  settingsHapticsLabel: '震感回饋',
  settingsHapticsHint: '銅錢落地與每爻成卦時的觸覺振動。',
  settingsCoinSkinLabel: '銅錢外觀',
  settingsCoinSkinHint: '預設為 Yaul Logo 方孔錢，與 App 圖示一致。',
  settingsCoinSkinDefault: 'Logo 預設',
  settingsCoinSkinCustomActive: '自訂錢面',
  settingsCoinSkinUpload: '上傳錢面圖片',
  settingsCoinSkinUploadHint: '圖片會完整適配顯示，不會裁剪。',
  settingsCoinSkinUploading: '上傳中…',
  settingsCoinSkinUploadFailedTitle: '上傳失敗',
  settingsCoinSkinUploadFailedBody: '無法儲存錢面圖片。請確認相簿權限，並盡量使用 JPEG 或 PNG。',
  settingsCoinSkinPermissionBody: '需要相簿權限才能選擇自訂錢面。',
  settingsCoinSkinNativeBody: '目前安裝包缺少相簿或圖片處理原生模組。請安裝重新建置的 App 後再試。',
  settingsCoinSkinFormatBody: '無法解析這張圖片。請選擇有效的 JPEG、PNG 或 HEIC 圖片。',
  settingsCoinSkinStorageBody: '規範化後的錢面圖片無法儲存到裝置空間。',
  settingsOpenSystemSettings: '開啟系統設定',
  settingsCoinSkinReset: '恢復 Logo 預設',
  settingsAccountSection: '帳號',
  settingsAccountAppleKind: 'Apple 帳號',
  settingsAccountSignedIn: '已登入',
  settingsDeleteAccount: '刪除帳號與資料',
  settingsDeleteAccountHint: '永久刪除帳號與解讀歷史，無法恢復。',
  settingsDeleteAccountTitle: '刪除帳號？',
  settingsDeleteAccountBody: '將永久刪除您的 HexAstral 帳號及 Yaul 解讀歷史。此操作無法撤銷。',
  settingsDeleteAccountContinue: '刪除',
  settingsDeleteAccountFailed: '刪除失敗，請稍後重試。',
  settingsDeleteAccountCancel: '取消',
  sceneLoading: '場景載入中…',
  settingsMemoryLabel: '個人化解讀記憶',
  settingsMemoryHint:
    '開啟後，僅 AI 深讀紀錄可能被檢索以豐富新的 AI 解讀；經典（免費）起卦不會寫入記憶。資料存於 HexAstral 伺服器，不出售；可隨時關閉。',
  settingsMemoryOn: '開啟',
  settingsMemoryOff: '關閉',
  settingsMemoryGuestHint: '使用 Apple 登入後可在多裝置間開啟記憶。',
  settingsMemorySaving: '儲存中…',
  beforeCastTitle: '占前必讀',
  beforeCastC1Title: '一事一問，誠意',
  beforeCastC1Body: '本輪只問一件事，起爻後勿改問。因疑而問、以誠相對、不為害人之事。',
  beforeCastC2Title: '靜心',
  beforeCastC2Body: '隨光點放慢呼吸，覺得穩了再下一步。',
  beforeCastC3Title: '開始',
  beforeCastC3Body: '點下方進入搖卦。',
  beforeCastNext: '下一步',
  beforeCastReady: '我已準備好',
  breathingOverlayText: '調匀呼吸……心念你的問題。',
  alertDuplicateTitle: '近期問過同一題',
  alertDuplicateMsg: '你在過去一天內問過這個完全相同的問題。若確有必要再問，請再確認心意。',
  alertDuplicateContinue: '仍要繼續',
  alertDuplicateCancel: '取消',
  alertRefusedTitle: '本次不予解讀',
  alertRefusedTradition: '易經傳統希望我們帶著誠意前來。',
  alertRefusedWarn: '若多次以輕佻或無關問題反覆占問，系統可能會短暫暫停 Yaul。',
  alertBannedTitle: 'Yaul 已暫停',
  alertBannedMsg: '暫停將在 {time} 後解除。請稍後再來，並以誠意發問。',
  alertFirstTitle: '請先閱讀',
  alertFirstMsg: '首次占問前，請先打開簡短說明。',
  alertFirstOpen: '打開',
  alertCooldownTitle: '請稍候',
  alertCooldownMsg: '距下次可占問還需約 {m} 分鐘。',
  alertAbortTitle: '確定中斷？',
  alertAbortMsg: '中途停止並不理想。你可選擇繼續，或清空後靜心再起。',
  alertContinue: '繼續',
  alertAbortConfirm: '仍要中斷',
  meHistoryTitle: '解讀歷史',
  meEmpty: '暫無歷史記錄',
  meViewAllHistory: '查看全部歷史',
  meSettings: '起卦與體感',
  meUpgrade: '升級專業版',
  restorePurchases: '恢復購買',
  mePrivacy: '隱私政策',
  meTerms: '使用條款',
  meSignOut: '登出',
  meSignInSectionTitle: '登入',
  meSignInHint: '使用 Apple 登入可在裝置間同步解讀歷史。',
  meAppleContinue: '透過 Apple 繼續',
  meApplePreparing: '正在準備登入…',
  meAppleUnavailable: 'Apple 登入需開發版或 App Store 版（Expo Go 不可用）。',
  meSignInIosOnly: '在 iOS 上可透過 Apple 登入同步歷史。',
  meSealByline: 'HexAstral 出品',
  mePromoBody: '前往主應用解鎖完整命盤與深度內容。',
  mePromoCta: '打開 HexAstral',
  stackMe: '我的',
  stackHistory: '歷史',
  stackResult: '結果',
  stackDetail: '詳解',
  stackCast: '起卦',
  stackPaywall: 'Yaul Pro',
  stackSettings: '設定',
  navBack: '返回',
  profileSignOutConfirm: '在此裝置上登出？帳號與歷史記錄仍保留在伺服器，除非您刪除帳號。',
  stackBeforeCast: '占前必讀',
  resultOpenDetail: '查看完整解讀',
  resultReadingId: '記錄編號：{id}',
  resultHexagramTitle: '第{num}卦 {name}',
  resultClassicalBadge: '經典解讀',
  resultUnlockAiCta: '解鎖 AI 深讀',
  upgradeAiCta: '將此卦升級為 AI 深讀',
  upgradeAiLoading: '升級中…',
  upgradeAiAlreadyDone: '已是 AI 解讀',
  historyTierClassical: '經典',
  historyTierAi: 'AI',
  detailTitle: '詳細解讀',
  detailFallbackAdvice: '把下一步做得簡單、可觀察。用行動回應焦慮。',
  detailClassicalAdviceHint:
    '本次為卦辭、爻辭與納甲結構的經典層解讀。購買釋讀包或 Pro 可在下一次起卦獲得 AI 解讀。',
  detailFallbackSummary: '守中而行。',
  detailClassicalTitle: '原文參考',
  detailNaJiaTitle: '納甲裝卦',
  detailJudgment: '卦辭',
  detailImage: '大象',
  detailLines: '爻辭',
  detailKeywords: '主題',
  resultFallbackInterpretation: '卦象已就緒，可進入全文解讀取得更多層次說明。',
  paywallUnlock: 'AI 深讀與 Pro',
  paywallRestore: '恢復購買',
  paywallRestoreRow: '恢復',
  paywallPlanCastPack1: '1 次 AI 釋讀',
  paywallPlanCastPack5: '5 次 AI 釋讀',
  paywallPlanCastPack10: '10 次 AI 釋讀',
  paywallPlanMonthly: '月度 Pro',
  paywallPlanAnnual: '年度 Pro',
  paywallLegalDisclaimer:
    '基於經典六爻與《周易》文化——僅供娛樂、文化研習與個人省思，不構成醫療、法律、財務或人生決策建議。',
  resultLegalDisclaimer: '本卦象解讀僅供文化探索與個人省思，非預測或專業建議。',
  detailLegalDisclaimer: '經典文獻與 AI 釋讀僅供研習與省思，非算命或決策建議。',
  devtoolsTitle: '開發者工具',
  devtoolsReplayOnboarding: '重置引導',
  devtoolsClearRitual: '清空占前確認與冷卻等本地狀態',
  devtoolsPortfolioUser: 'Portfolio 用戶',
  devtoolsShowEnv: '查看建置資訊',
  devtoolsDone: '已儲存。',
  devtoolsOk: '好',
  castTitle: '你的問題',
  castPlaceholder: '這週我最該留意什麼？',
  castSubmit: '起卦',
  castBack: '返回',
  castError: '生成解讀失敗，請重試。',
  homeError: '生成解讀失敗，請重試。',
  alertQuotaTitle: '解讀次數',
  alertQuotaMsg: '本月免費經典起卦次數已用完。購買 AI 釋讀包或 Pro 可獲得個人化解讀。',
  alertQuotaGuestDailyMsg:
    '今日（UTC）三次免登入經典起卦已用完。請使用 Apple 登入以繼續使用帳號額度，或購買 AI 釋讀包。',
  alertQuotaSignIn: '前往登入',
  alertQuotaUpgrade: '查看方案',
  motionUnavailableTitle: '感應不可用',
  motionUnavailableMessage: '目前裝置無法執行運動驅動的銅錢物理。可重試感應，或明確選擇數字代搖。',
  motionPermissionMessage: '尚未取得運動感測器權限。請在系統設定中開啟「運動與健身」權限後重試。',
  motionInsufficientMessage:
    '本次運動序列不完整或動作過輕，無法形成運動因果的一爻。請合掌再搖一輪，鬆手開掌後重試。',
  motionInterruptedMessage: '本爻感應尚未完成便被中斷。請保持 Yaul 在前景並重試。',
  motionPhysicsTimeoutMessage: '銅錢未在時限內形成可判讀的穩定落面。請合掌再搖。',
  motionRetry: '再搖',
  motionRetryPreferredTitle: '請再搖一次',
  motionRetryPreferredHint: '連續失敗數次後才會出現數字代搖；成功路徑永不注入代搖。',
  shakeHoldLonger: '請再合掌搖一會兒、帶上真實晃動，再鬆手開掌。',
  lineSeenTitle: '看清落子',
  lineSeenMessage: '可先按落子原樣讀字背；看清後再可選整齊成排。',
  lineSeenAsIs: '保持落形',
  lineSeenNeat: '整齊排列',
  digitalAssistAction: '數字代搖',
  digitalAssistTitle: '使用數字代搖？',
  digitalAssistMessage: '這一爻將使用安全數字隨機，而不是你的運動；來源會明確記錄為「數字代搖」。',
  digitalAssistConfirm: '確認數字代搖',
  waYingTitle: '外應 · 本卦不作數',
  waYingMessage:
    '有銅錢立於桌緣、難以定陰陽。依傳統六爻以外應論之——本卦整卦作廢（含已搖出之爻），不可軟失敗進入下一爻。請靜心後從初爻重新搖齊六爻。此象多示所問懸而未決、變數未落，不必強判吉凶。',
  waYingAck: '從初爻重搖',
  deskBenLabel: '本卦',
  deskBianLabel: '變卦',
  deskHuLabel: '互卦',
  deskPalace: '{palace}宮',
  deskShiYing: '世 {shi} · 應 {ying}',
  deskTableTitle: '裝卦表',
  deskColYao: '爻',
  deskColGanZhi: '納甲',
  deskColLiuQin: '六親',
  deskColMark: '標記',
  deskMarkShi: '世',
  deskMarkYing: '應',
  deskMarkDong: '動',
  deskMarkKong: '空',
  deskNoBian: '無動爻 · 無變卦',
  onboardKicker: 'HEXASTRAL SATELLITE',
  onboardSubtitle:
    '三枚銅錢，六爻成卦。Yaul 是易經六爻研習工具——誠心問一事，擲錢起一卦，閱讀經典或 AI 釋讀以供省思。',
  onboardGetStarted: '開始',
  onboardContinue: '繼續',
  onboardBack: '返回',
}

const JA: Record<keyof typeof EN, string> = {
  homeShake: '振る',
  homeHoldShake: '長押しで振る',
  homeReleaseOpen: '離して開掌',
  homeCast: '占う',
  homeCastingButton: '解釈中…',
  homeInputA11y: 'この占いの質問',
  homeQuestionPrompt: 'タップして質問を入力',
  questionSheetTitle: 'あなたの質問',
  questionSheetCta: '占いを始める',
  homeAbortA11y: '占いを中断',
  homeProgressA11y: '{n} / 6 爻',
  homeYaoCoinBarLegend: '左の三マークは銭の向き。右の棒は易の爻（7/8は多数決ではない）。',
  homeYaoRowA11y: '第{n}爻：{faces}。合計{total}。',
  homeYaoPosition: '爻位',
  homeYaoThrow: '合計',
  coinFaceA11yYin: '陰の面',
  coinFaceA11yYang: '陽の面',
  homeGlFallback: '3D は WebGL 付きビルドが必要。振りと爻の表示はそのまま使えます。',
  homeDevShakeHint: '開発ビルドは下の「振る」を使ってください。',
  settingsTitle: '設定',
  settingsMotionLabel: '振って爻を出す',
  settingsMotionHint: 'オンにすると、強く振るたびに三銭が一度分落ちたのと同じになります。',
  settingsMotionOn: 'オン',
  settingsMotionOff: 'オフ',
  settingsSensorySection: '起卦と体感',
  settingsHapticsLabel: '振動フィードバック',
  settingsHapticsHint: '銭が着地したときと各爻が確定したときに振動します。',
  settingsCoinSkinLabel: '銭の外観',
  settingsCoinSkinHint: '既定は Yaul ロゴの方孔銭（アプリアイコンと同じ形）。',
  settingsCoinSkinDefault: 'ロゴ既定',
  settingsCoinSkinCustomActive: 'カスタム銭面',
  settingsCoinSkinUpload: '銭面画像をアップロード',
  settingsCoinSkinUploadHint: '画像は切り取らず、全体が収まるよう調整されます。',
  settingsCoinSkinUploading: 'アップロード中…',
  settingsCoinSkinUploadFailedTitle: 'アップロード失敗',
  settingsCoinSkinUploadFailedBody:
    '銭面画像を保存できませんでした。写真へのアクセスを確認し、JPEG / PNG をお試しください。',
  settingsCoinSkinPermissionBody: 'カスタム銭面を選ぶには写真へのアクセスが必要です。',
  settingsCoinSkinNativeBody:
    'このビルドには写真または画像処理のネイティブモジュールが含まれていません。再ビルドしたアプリをインストールしてください。',
  settingsCoinSkinFormatBody:
    'この画像を読み込めませんでした。有効な JPEG、PNG、または HEIC を選んでください。',
  settingsCoinSkinStorageBody: '正規化した銭面画像を端末ストレージに保存できませんでした。',
  settingsOpenSystemSettings: '設定を開く',
  settingsCoinSkinReset: 'ロゴ既定に戻す',
  settingsAccountSection: 'アカウント',
  settingsAccountAppleKind: 'Apple アカウント',
  settingsAccountSignedIn: 'ログイン済み',
  settingsDeleteAccount: 'アカウントとデータを削除',
  settingsDeleteAccountHint: 'アカウントと占い履歴を完全に削除します。元に戻せません。',
  settingsDeleteAccountTitle: 'アカウントを削除しますか？',
  settingsDeleteAccountBody:
    'HexAstral アカウントと Yaul の占い履歴がサーバーから完全に削除されます。取り消せません。',
  settingsDeleteAccountContinue: '削除',
  settingsDeleteAccountFailed: '削除できませんでした。もう一度お試しください。',
  settingsDeleteAccountCancel: 'キャンセル',
  sceneLoading: 'シーンを読み込み中…',
  settingsMemoryLabel: 'パーソナライズされた占いメモリー',
  settingsMemoryHint:
    'オンにすると、過去の AI 深読の短い要約のみを参照して新しい AI 解釈を豊かにします。クラシック（無料）占いは記憶に保存されません。データは HexAstral サーバーに保存され、販売されません。いつでもオフにできます。',
  settingsMemoryOn: 'オン',
  settingsMemoryOff: 'オフ',
  settingsMemoryGuestHint: 'Apple でサインインするとデバイス間で有効にできます。',
  settingsMemorySaving: '保存中…',
  beforeCastTitle: '占う前に',
  beforeCastC1Title: '一問一占',
  beforeCastC1Body:
    'この回は一つの問いに集中し、爻が始まったら変えない。誠実に、他を傷つけないために。',
  beforeCastC2Title: '整える',
  beforeCastC2Body: '点のリズムで呼吸をゆるめ、整ったら次へ。',
  beforeCastC3Title: '始める',
  beforeCastC3Body: '下のボタンで占いに入ります。',
  beforeCastNext: '次へ',
  beforeCastReady: '準備できました',
  breathingOverlayText: '呼吸を整え……問いを保ちます。',
  alertDuplicateTitle: '最近と同じ問い',
  alertDuplicateMsg:
    '過去24時間以内に、まったく同じ問いを占っています。本当に改めて問う場合だけ続けてください。',
  alertDuplicateContinue: 'それでも続ける',
  alertDuplicateCancel: 'キャンセル',
  alertRefusedTitle: '今回は解釈しません',
  alertRefusedTradition: '易は誠実な心で臨むことを求めます。',
  alertRefusedWarn: '軽い・無意味な問いを繰り返すと、Yaul が一時停止されることがあります。',
  alertBannedTitle: 'Yaul を一時停止中',
  alertBannedMsg: '{time} までお待ちください。誠実な問いとともに、また来てください。',
  alertFirstTitle: '先にお読みください',
  alertFirstMsg: '初めて占う前に、短い案内を開いてください。',
  alertFirstOpen: '開く',
  alertCooldownTitle: '少しお待ちください',
  alertCooldownMsg: '次の占いまであと約 {m} 分お待ちください。',
  alertAbortTitle: '中断しますか？',
  alertAbortMsg:
    '途中で止めるのは望ましくありません。続けるか、心を整えて最初からやり直してください。',
  alertContinue: '続ける',
  alertAbortConfirm: 'それでも中断',
  meHistoryTitle: '履歴',
  meEmpty: '履歴はまだありません',
  meViewAllHistory: 'すべての履歴を見る',
  meSettings: '起卦とモーション',
  meUpgrade: 'Pro にアップグレード',
  restorePurchases: '購入を復元',
  mePrivacy: 'プライバシーポリシー',
  meTerms: '利用規約',
  meSignOut: 'サインアウト',
  meSignInSectionTitle: 'サインイン',
  meSignInHint: 'Apple でサインインすると、解釈履歴を端末間で同期できます。',
  meAppleContinue: 'Apple で続ける',
  meApplePreparing: 'サインインを準備中…',
  meAppleUnavailable:
    'Apple サインインは開発ビルドまたは App Store 版が必要です（Expo Go では使えません）。',
  meSignInIosOnly: '履歴の同期は iOS の Apple サインインで利用できます。',
  meSealByline: 'by HexAstral',
  mePromoBody: '本編アプリで完全な運勢チャートを解放。',
  mePromoCta: 'HexAstral を開く',
  stackMe: 'マイ',
  stackHistory: '履歴',
  stackResult: '結果',
  stackDetail: '詳細',
  stackCast: '占い',
  stackPaywall: 'Yaul Pro',
  stackSettings: '設定',
  navBack: '戻る',
  profileSignOutConfirm:
    'この端末からサインアウトしますか？アカウント削除まで履歴はサーバーに残ります。',
  stackBeforeCast: '占う前に',
  resultOpenDetail: '全文の解釈を見る',
  resultReadingId: '記録 ID: {id}',
  resultHexagramTitle: '卦 {num} {name}',
  resultClassicalBadge: '古典解釈',
  resultUnlockAiCta: 'AI 深読を解放',
  upgradeAiCta: 'この占いを AI 深読にアップグレード',
  upgradeAiLoading: 'アップグレード中…',
  upgradeAiAlreadyDone: 'すでに AI',
  historyTierClassical: 'クラシック',
  historyTierAi: 'AI',
  detailTitle: '詳しい解釈',
  detailFallbackAdvice: '次の一歩を小さく、確かめられる形に。行動が不安に答えます。',
  detailClassicalAdviceHint:
    '今回は卦辞・爻辞・納甲の古典層です。次の起卦で AI 解釈を得るには占いパックまたは Pro をご利用ください。',
  detailFallbackSummary: '中心を保ち、進む。',
  detailClassicalTitle: '原典の参照',
  detailNaJiaTitle: '納甲装卦',
  detailJudgment: '卦辞',
  detailImage: '大象',
  detailLines: '爻辞',
  detailKeywords: 'テーマ',
  resultFallbackInterpretation: '卦が出揃いました。全文の解釈で深い読み解きへ。',
  paywallUnlock: 'AI 解釈と Pro',
  paywallRestore: '購入を復元',
  paywallRestoreRow: '復元',
  paywallPlanCastPack1: 'AI 1回',
  paywallPlanCastPack5: 'AI 5回',
  paywallPlanCastPack10: 'AI 10回',
  paywallPlanMonthly: '月額 Pro',
  paywallPlanAnnual: '年額 Pro',
  paywallLegalDisclaimer:
    '古典六爻・周易文化に基づく娯楽・文化研習・個人的省思用。医療・法律・財務・人生判断の助言ではありません。',
  resultLegalDisclaimer:
    'この卦の解釈は文化探索と省思のためのもので、予測や専門的助言ではありません。',
  detailLegalDisclaimer:
    '古典文献と AI 解釈は研習と省思用であり、占いや意思決定の助言ではありません。',
  devtoolsTitle: '開発者ツール',
  devtoolsReplayOnboarding: 'オンボーディングをリセット',
  devtoolsClearRitual: '儀式・クールダウン等のローカル設定を消去',
  devtoolsPortfolioUser: 'Portfolio ユーザー',
  devtoolsShowEnv: 'ビルド情報を表示',
  devtoolsDone: '保存しました。',
  devtoolsOk: 'OK',
  castTitle: 'あなたの問い',
  castPlaceholder: '今週、何に気をつけるべき？',
  castSubmit: '占う',
  castBack: '戻る',
  castError: '解釈の生成に失敗しました。再試行してください。',
  homeError: '解釈の生成に失敗しました。再試行してください。',
  alertQuotaTitle: '利用上限',
  alertQuotaMsg:
    '今月の無料古典起卦回数に達しました。AI 占いパックまたは Pro で個別解釈をご利用ください。',
  alertQuotaGuestDailyMsg:
    '本日（UTC）のゲスト向け無料古典起卦3回を使い切りました。Appleでサインインするか、AI 占いパックをご購入ください。',
  alertQuotaSignIn: 'サインイン',
  alertQuotaUpgrade: 'プランを見る',
  motionUnavailableTitle: '感応を利用できません',
  motionUnavailableMessage:
    'この端末では動きで駆動する硬貨物理を実行できません。感応を再試行するか、デジタル代行を明示的に選んでください。',
  motionPermissionMessage:
    'モーションへのアクセスが許可されていません。設定で「モーションとフィットネス」を有効にして再試行してください。',
  motionInsufficientMessage:
    '動きの記録が不完全、または弱すぎて運動由来の爻を作れませんでした。合掌でもう一度振り、離して開掌して再試行してください。',
  motionInterruptedMessage:
    'この爻の感応が完了前に中断されました。Yaul を前面に保って再試行してください。',
  motionPhysicsTimeoutMessage:
    '硬貨が判読可能な静止状態に達しませんでした。合掌でもう一度振ってください。',
  motionRetry: 'もう一度振る',
  motionRetryPreferredTitle: 'もう一度振ってください',
  motionRetryPreferredHint:
    '連続して失敗したあとでのみデジタル代行が出ます。成功経路に代行は入りません。',
  shakeHoldLonger: 'もう少し合掌して、しっかり動かしてから離して開掌してください。',
  lineSeenTitle: '落ちた硬貨を読む',
  lineSeenMessage: '落ちたままの形で字背を読み、確認後に整列も選べます。',
  lineSeenAsIs: 'そのまま',
  lineSeenNeat: '整列する',
  digitalAssistAction: 'デジタル代行',
  digitalAssistTitle: 'デジタル代行を使いますか？',
  digitalAssistMessage:
    'この爻はあなたの動きではなく、安全なデジタル乱数を使います。出所はデジタル代行として記録されます。',
  digitalAssistConfirm: 'デジタル代行を使う',
  waYingTitle: '外応 — この卦は無効',
  waYingMessage:
    '硬貨が縁に立ち、陰陽を定められませんでした。伝統的六爻では卦全体が無効（すでに出た爻も含む）とし、次の爻へ進みません。心を整えて初爻から六爻を取り直します。問いが宙吊り・未決であることのサインとされることが多いです。',
  waYingAck: '初爻からやり直す',
  deskBenLabel: '本卦',
  deskBianLabel: '変卦',
  deskHuLabel: '互卦',
  deskPalace: '{palace}宮',
  deskShiYing: '世 {shi} · 応 {ying}',
  deskTableTitle: '納甲表',
  deskColYao: '爻',
  deskColGanZhi: '納甲',
  deskColLiuQin: '六親',
  deskColMark: '標',
  deskMarkShi: '世',
  deskMarkYing: '応',
  deskMarkDong: '動',
  deskMarkKong: '空',
  deskNoBian: '動爻なし · 変卦なし',
  onboardKicker: 'HEXASTRAL SATELLITE',
  onboardSubtitle:
    '三枚の硬貨で六爻を立てる。Yaul は易経六爻の研習ジャーナル——一つの誠実な問いを持ち、卦を起こし、古典または AI 解釈を省思のために読む。',
  onboardGetStarted: '始める',
  onboardContinue: '続ける',
  onboardBack: '戻る',
}

const KO: Record<keyof typeof EN, string> = {
  homeShake: '흔들기',
  homeHoldShake: '길게 눌러 흔들기',
  homeReleaseOpen: '손을 떼어 개장',
  homeCast: '점치기',
  homeCastingButton: '해석 중…',
  homeInputA11y: '이번 점의 질문',
  homeQuestionPrompt: '탭하여 질문을 입력하세요',
  questionSheetTitle: '당신의 질문',
  questionSheetCta: '점 시작하기',
  homeAbortA11y: '이번 점 중단',
  homeProgressA11y: '여섯 효 중 {n}효 완료',
  homeYaoCoinBarLegend:
    '세 개의 작은 표시는 동전 면입니다. 효는 주역 작괘법을 따르며 7/8은 다수결로 음양을 정하지 않습니다.',
  homeYaoRowA11y: '제{n}효: {faces}. 합계 {total}.',
  homeYaoPosition: '효위',
  homeYaoThrow: '합계',
  coinFaceA11yYin: '음면',
  coinFaceA11yYang: '양면',
  homeGlFallback:
    '3D 미리보기에는 WebGL 개발 빌드가 필요합니다. 던지기는 정상 작동하며, 효는 던질 때마다 표시됩니다.',
  homeDevShakeHint:
    '개발 빌드: 아래 「흔들기」 버튼을 사용하세요(시스템이 흔들기를 개발 메뉴로 예약할 수 있습니다).',
  settingsTitle: '설정',
  settingsMotionLabel: '흔들어 점치기',
  settingsMotionHint: '켜면 세게 흔드는 동작이 세 동전을 한 번 던진 것으로 간주됩니다.',
  settingsMotionOn: '켜짐',
  settingsMotionOff: '꺼짐',
  settingsSensorySection: '기卦와 체감',
  settingsHapticsLabel: '진동 피드백',
  settingsHapticsHint: '동전이 떨어지거나 각 효가 확정될 때 진동합니다.',
  settingsCoinSkinLabel: '동전 외관',
  settingsCoinSkinHint: '기본은 Yaul 로고 방공전 — 앱 아이콘과 같은 형태입니다.',
  settingsCoinSkinDefault: '로고 기본',
  settingsCoinSkinCustomActive: '사용자 동전 면',
  settingsCoinSkinUpload: '동전 면 이미지 업로드',
  settingsCoinSkinUploadHint: '이미지는 잘리지 않고 전체가 맞춰 표시됩니다.',
  settingsCoinSkinUploading: '업로드 중…',
  settingsCoinSkinUploadFailedTitle: '업로드 실패',
  settingsCoinSkinUploadFailedBody:
    '동전 면 이미지를 저장할 수 없습니다. 사진 접근 권한을 확인하고 JPEG/PNG를 사용해 보세요.',
  settingsCoinSkinPermissionBody: '사용자 동전 면을 선택하려면 사진 접근 권한이 필요합니다.',
  settingsCoinSkinNativeBody:
    '이 빌드에는 사진 또는 이미지 처리 네이티브 모듈이 없습니다. 새로 빌드한 앱을 설치한 뒤 다시 시도하세요.',
  settingsCoinSkinFormatBody:
    '이 이미지를 해석할 수 없습니다. 올바른 JPEG, PNG 또는 HEIC 이미지를 선택하세요.',
  settingsCoinSkinStorageBody: '정규화된 동전 이미지를 기기 저장소에 저장할 수 없습니다.',
  settingsOpenSystemSettings: '설정 열기',
  settingsCoinSkinReset: '로고 기본으로 복원',
  settingsAccountSection: '계정',
  settingsAccountAppleKind: 'Apple 계정',
  settingsAccountSignedIn: '로그인됨',
  settingsDeleteAccount: '계정 및 데이터 삭제',
  settingsDeleteAccountHint: '계정과 해석 기록을 영구 삭제합니다. 되돌릴 수 없습니다.',
  settingsDeleteAccountTitle: '계정을 삭제할까요?',
  settingsDeleteAccountBody:
    'HexAstral 계정과 Yaul 해석 기록이 서버에서 영구 삭제됩니다. 취소할 수 없습니다.',
  settingsDeleteAccountContinue: '삭제',
  settingsDeleteAccountFailed: '삭제하지 못했습니다. 다시 시도해 주세요.',
  settingsDeleteAccountCancel: '취소',
  sceneLoading: '장면 불러오는 중…',
  settingsMemoryLabel: '개인화 해석 메모리',
  settingsMemoryHint:
    '켜면 과거 AI 심층 해석 요약만 불러와 새 AI 해석을 풍부하게 합니다. 클래식(무료) 점괘는 기억에 저장되지 않습니다. 데이터는 HexAstral 서버에 저장되며 판매되지 않습니다. 언제든지 끌 수 있습니다.',
  settingsMemoryOn: '켜짐',
  settingsMemoryOff: '꺼짐',
  settingsMemoryGuestHint: 'Apple로 로그인하면 여러 기기에서 메모리를 사용할 수 있습니다.',
  settingsMemorySaving: '저장 중…',
  beforeCastTitle: '점 보기 전에',
  beforeCastC1Title: '한 가지 질문, 정성스러운 마음',
  beforeCastC1Body:
    '이번에는 한 가지 질문에 집중하고, 효가 시작되면 바꾸지 마세요. 진정한 의문으로, 정직하게, 남을 해치지 않는 일을 물으세요.',
  beforeCastC2Title: '마음 가다듬기',
  beforeCastC2Body: '빛의 박동에 맞춰 천천히 호흡하고, 안정되면 다음으로 넘어가세요.',
  beforeCastC3Title: '준비 완료',
  beforeCastC3Body: '아래를 탭하여 점을 시작하세요.',
  beforeCastNext: '다음',
  beforeCastReady: '준비되었습니다',
  breathingOverlayText: '호흡을 가다듬고… 질문에 마음을 두세요.',
  alertDuplicateTitle: '최근 같은 질문',
  alertDuplicateMsg:
    '지난 하루 안에 똑같은 질문을 하셨습니다. 정말 다시 묻고자 할 때만 계속하세요.',
  alertDuplicateContinue: '그래도 계속',
  alertDuplicateCancel: '취소',
  alertRefusedTitle: '이번에는 해석하지 않습니다',
  alertRefusedTradition: '역경은 정성스러운 마음으로 다가올 것을 청합니다.',
  alertRefusedWarn: '가볍거나 무관한 질문을 반복하면 Yaul가 잠시 중단될 수 있습니다.',
  alertBannedTitle: 'Yaul 일시 중단',
  alertBannedMsg: '이 중단은 {time} 후에 해제됩니다. 정성스러운 질문과 함께 다시 찾아주세요.',
  alertFirstTitle: '먼저 읽어 주세요',
  alertFirstMsg: '첫 점을 보기 전에 짧은 안내를 한 번 열어 주세요.',
  alertFirstOpen: '열기',
  alertCooldownTitle: '잠시 기다려 주세요',
  alertCooldownMsg: '새 해석까지 약 {m}분 더 기다려 주세요.',
  alertAbortTitle: '점을 중단할까요?',
  alertAbortMsg:
    '도중에 멈추는 것은 권장되지 않습니다. 초기화하고 마음을 가다듬어 다시 시작할 수 있습니다.',
  alertContinue: '계속',
  alertAbortConfirm: '그래도 중단',
  meHistoryTitle: '해석 기록',
  meEmpty: '아직 점 기록이 없습니다.',
  meViewAllHistory: '전체 기록 보기',
  meSettings: '점·모션 설정',
  meUpgrade: 'Pro로 업그레이드',
  restorePurchases: '구매 복원',
  mePrivacy: '개인정보 처리방침',
  meTerms: '이용약관',
  meSignOut: '로그아웃',
  meSignInSectionTitle: '로그인',
  meSignInHint: 'Apple로 로그인하면 기기 간 해석 기록을 동기화할 수 있습니다.',
  meAppleContinue: 'Apple로 계속하기',
  meApplePreparing: '로그인 준비 중…',
  meAppleUnavailable:
    'Apple 로그인은 개발 빌드 또는 App Store 빌드가 필요합니다(Expo Go에서는 사용할 수 없습니다).',
  meSignInIosOnly: 'iOS에서 Apple 로그인으로 기록을 동기화할 수 있습니다.',
  meSealByline: 'HexAstral 제공',
  mePromoBody: '대표 앱에서 완전한 명반을 잠금 해제하세요.',
  mePromoCta: 'HexAstral 열기',
  stackMe: '내 정보',
  stackHistory: '기록',
  stackResult: '결과',
  stackDetail: '해석',
  stackCast: '점치기',
  stackPaywall: 'Yaul Pro',
  stackSettings: '설정',
  navBack: '뒤로',
  profileSignOutConfirm:
    '이 기기에서 로그아웃할까요? 계정을 삭제하기 전까지 기록은 서버에 남습니다.',
  stackBeforeCast: '점 보기 전에',
  resultOpenDetail: '전체 해석 보기',
  resultReadingId: '기록 번호: {id}',
  resultHexagramTitle: '제{num}괘 {name}',
  resultClassicalBadge: '고전 해석',
  resultUnlockAiCta: 'AI 심층 해석 잠금 해제',
  upgradeAiCta: '이 점괘를 AI 심층 해석으로 업그레이드',
  upgradeAiLoading: '업그레이드 중…',
  upgradeAiAlreadyDone: '이미 AI',
  historyTierClassical: '클래식',
  historyTierAi: 'AI',
  detailTitle: '상세 해석',
  detailFallbackAdvice: '다음 한 걸음을 단순하고 관찰 가능하게 만드세요. 행동이 불안에 답합니다.',
  detailClassicalAdviceHint:
    '이번 결과는 괘사·효사·납갑 구조의 고전층입니다. 다음 점에서 AI 해석을 받으려면 점 패키지 또는 Pro를 구매하세요.',
  detailFallbackSummary: '중심을 지키며 나아가세요.',
  detailClassicalTitle: '원문 참조',
  detailNaJiaTitle: '납갑 장괘',
  detailJudgment: '괘사',
  detailImage: '대상',
  detailLines: '효사',
  detailKeywords: '주제',
  resultFallbackInterpretation:
    '괘가 준비되었습니다. 전체 해석에서 장(章) 형식의 안내를 확인하세요.',
  paywallUnlock: 'AI 해석 및 Pro',
  paywallRestore: '구매 복원',
  paywallRestoreRow: '복원',
  paywallPlanCastPack1: 'AI 1회',
  paywallPlanCastPack5: 'AI 5회',
  paywallPlanCastPack10: 'AI 10회',
  paywallPlanMonthly: '월간 Pro',
  paywallPlanAnnual: '연간 Pro',
  paywallLegalDisclaimer:
    '고전 육효·주역 문화 기반 — 오락, 문화 학습, 개인적 성찰용입니다. 의료·법률·재정·인생 결정 조언이 아닙니다.',
  resultLegalDisclaimer: '이 괘 해석은 문화 탐구와 성찰용이며, 예측이나 전문 조언이 아닙니다.',
  detailLegalDisclaimer:
    '고전 문헌과 AI 해석은 학습·성찰용이며, 점술이나 의사결정 조언이 아닙니다.',
  devtoolsTitle: '개발자 도구',
  devtoolsReplayOnboarding: '온보딩 초기화',
  devtoolsClearRitual: '의식·쿨다운 등 로컬 설정 지우기',
  devtoolsPortfolioUser: 'Portfolio 사용자',
  devtoolsShowEnv: '빌드 정보 보기',
  devtoolsDone: '로컬에 저장되었습니다.',
  devtoolsOk: '확인',
  castTitle: '당신의 질문',
  castPlaceholder: '이번 주에 무엇을 주의해야 할까요?',
  castSubmit: '괘 만들기',
  castBack: '뒤로',
  castError: '해석 생성에 실패했습니다. 다시 시도해 주세요.',
  homeError: '해석 생성에 실패했습니다. 다시 시도해 주세요.',
  alertQuotaTitle: '해석 횟수 제한',
  alertQuotaMsg:
    '이번 달 무료 고전 점 횟수를 모두 사용했습니다. AI 점 패키지 또는 Pro로 맞춤 해석을 받으세요.',
  alertQuotaGuestDailyMsg:
    '오늘(UTC) 비로그인 고전 점 3회를 모두 사용했습니다. Apple로 로그인하거나 AI 점 패키지를 구매하세요.',
  alertQuotaSignIn: '로그인',
  alertQuotaUpgrade: '요금제 보기',
  motionUnavailableTitle: '감응을 사용할 수 없음',
  motionUnavailableMessage:
    '이 기기에서는 움직임으로 구동되는 동전 물리를 실행할 수 없습니다. 감응을 다시 시도하거나 디지털 대행을 명시적으로 선택하세요.',
  motionPermissionMessage:
    '동작 센서 권한이 허용되지 않았습니다. 설정에서 동작 및 피트니스 접근을 켠 뒤 다시 시도하세요.',
  motionInsufficientMessage:
    '동작 기록이 불완전하거나 너무 약해 움직임에 의한 효를 만들 수 없습니다. 손을 모으고 다시 흔든 뒤 손을 떼어 개장하세요.',
  motionInterruptedMessage:
    '이 효의 감응이 완료되기 전에 중단되었습니다. Yaul을 전면에 유지하고 다시 시도하세요.',
  motionPhysicsTimeoutMessage:
    '동전이 판독 가능한 안정 상태에 도달하지 못했습니다. 손을 모으고 다시 흔드세요.',
  motionRetry: '다시 흔들기',
  motionRetryPreferredTitle: '다시 흔들어 주세요',
  motionRetryPreferredHint:
    '연속 실패 후에만 디지털 대행이 나타납니다. 성공 경로에는 대행이 들어가지 않습니다.',
  shakeHoldLonger: '조금 더 손을 모으고 실제로 흔든 뒤 손을 떼어 개장하세요.',
  lineSeenTitle: '떨어진 동전 읽기',
  lineSeenMessage: '떨어진 그대로 자/배를 읽고, 확인 후 정렬할 수도 있습니다.',
  lineSeenAsIs: '떨어진 대로',
  lineSeenNeat: '정렬하기',
  digitalAssistAction: '디지털 대행',
  digitalAssistTitle: '디지털 대행을 사용할까요?',
  digitalAssistMessage:
    '이 효는 사용자의 움직임이 아닌 안전한 디지털 난수를 사용하며, 출처가 디지털 대행으로 기록됩니다.',
  digitalAssistConfirm: '디지털 대행 사용',
  waYingTitle: '외응 · 이 괘는 무효',
  waYingMessage:
    '동전이 모서리에 서서 음양을 정할 수 없습니다. 전통 육효에서는 괘 전체가 무효(이미 나온 효 포함)가 되며, 다음 효로 넘어가지 않습니다. 마음을 가다듬고 첫 효부터 여섯 효를 다시 만드세요. 흔히 일이 미결이거나 변수가 정해지지 않았음을 암시하며, 길흉을 억지로 단정할 필요는 없습니다.',
  waYingAck: '초효부터 다시',
  deskBenLabel: '본괘',
  deskBianLabel: '변괘',
  deskHuLabel: '호괘',
  deskPalace: '{palace}궁',
  deskShiYing: '세 {shi} · 응 {ying}',
  deskTableTitle: '장괘표',
  deskColYao: '효',
  deskColGanZhi: '납갑',
  deskColLiuQin: '육친',
  deskColMark: '표기',
  deskMarkShi: '세',
  deskMarkYing: '응',
  deskMarkDong: '동',
  deskMarkKong: '공',
  deskNoBian: '동효 없음 · 변괘 없음',
  onboardKicker: 'HEXASTRAL SATELLITE',
  onboardSubtitle:
    '동전 셋을 던져 여섯 효로 괘를 세웁니다. Yaul는 주역 육효 학습 저널입니다 — 한 가지 진심 어린 질문을 하고, 괘를 세운 뒤, 고전 또는 AI 해석을 성찰용으로 읽습니다.',
  onboardGetStarted: '시작하기',
  onboardContinue: '계속',
  onboardBack: '뒤로',
}

const DICT: Record<string, Record<keyof typeof EN, string>> = {
  en: EN,
  zh: ZH,
  'zh-Hant': ZH_HANT,
  ja: JA,
  ko: KO,
}

function normalizeLocale(input: string): UiLocale {
  if (input in DICT) return input as UiLocale
  if (input === 'zh-CN' || input.startsWith('zh-CN')) return 'zh'
  if (input.startsWith('zh-Hant') || input === 'zh-TW' || input === 'zh-HK') return 'zh-Hant'
  if (input.startsWith('zh')) return 'zh'
  const base = input.split('-')[0] ?? 'en'
  if (base in DICT) return base as UiLocale
  return 'en'
}

export type SatelliteLocaleKey = keyof typeof EN

export function useSatelliteI18n() {
  const [locale, setLocale] = useState<UiLocale>('en')

  useEffect(() => {
    ;(async () => {
      const saved = await AsyncStorage.getItem(LOCALE_STORAGE_KEY)
      setLocale(resolveUiLocale(saved))
    })()
  }, [])

  const t = (key: SatelliteLocaleKey, vars?: Record<string, string | number>): string => {
    const row = DICT[locale] ?? DICT.en
    const base = (row?.[key] ?? EN[key] ?? key) as string
    if (!vars) return base
    return Object.entries(vars).reduce((acc, [k, v]) => acc.replaceAll(`{${k}}`, String(v)), base)
  }

  /** API / backend locale tag (svc-astro i18n-prompt supports en/zh/zh-Hant/ja/ko) */
  const apiLocale =
    locale === 'zh'
      ? 'zh'
      : locale === 'zh-Hant'
        ? 'zh-Hant'
        : locale === 'ja'
          ? 'ja'
          : locale === 'ko'
            ? 'ko'
            : 'en'

  return { locale: apiLocale, uiLocale: locale, t }
}

...
export function BirthInfoForm({
  ...
  return (
    ...
    >
      {label && (
        <p style={{ color: 'var(--color-ivory-dim)', textAlign: 'center', marginBottom: '0.5rem' }}>
          {label}
        </p>
      )}
      ...
      <div>
        ...
        <CityAutocomplete
          ...
          onChange={(city: string, geo?: CitySelection) =>
            onChange({
              ...value,
              birthCity: city,
              latitude: geo?.latitude ?? value.latitude,
              longitude: geo?.longitude ?? value.longitude,
              timezone: geo?.timezone ?? value.timezone,
            })
          }
          placeholder={t('cityPlaceholder')}
          style={{
            ...fieldStyle,
            borderColor: value.birthCity.trim() ? 'var(--color-gold)' : 'var(--color-border)',
          }}
        />
      </div>
      ...
      <div>
        ...
        >
          <label style={{ fontSize: '0.8rem', color: 'var(--color-gold)' }}>
            {value.isLunar ? t('dateLabelLunar') : t('dateLabel')}
          </label>
        ...
        {value.isLunar ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem' }}>
            <select
              style={fieldStyle}
              value={ly}
              onChange={(e) => onChange({ ...value, solarDate: `${e.target.value}-${lm}-${ld}` })}
            >
              {years.map((y) => (
            ...
            <select
              ...
              value={lm}
              onChange={(e) =>
                onChange({ ...value, solarDate: `${ly}-${e.target.value.padStart(2, '0')}-${ld}` })
              }
            >
              {months.map((m) => (
            ...
            <select
              ...
              onChange={(e) =>
                onChange({
                  ...value,
                  solarDate: `${ly}-${lm}-${String(e.target.value).padStart(2, '0')}`,
                })
              }
            >
              {days.map((d) => (
          ...
          <input
            ...
            onChange={(e) => onChange({ ...value, solarDate: e.target.value })}
      ...
      <div>
        ...
        >
          ...
          <label style={toggleStyle}>
            <Switch
              ...
              onChange={(c) => onChange({ ...value, useTrueSolarTime: c })}
            />
            {t('checkTrueSolar')}
          </label>
        </div>
        ...
        {value.useTrueSolarTime ? (
          <input
            ...
            onChange={(e) => onChange({ ...value, exactTime: e.target.value })}
            style={{ ...fieldStyle, colorScheme: 'dark' }}
          />
        ) : (
          <select
            value={value.timeIndex}
            onChange={(e) => onChange({ ...value, timeIndex: Number(e.target.value) })}
            style={fieldStyle}
          >
            {shichenLabels.map((label, idx) => (
        ...
        >
          {(['male', 'female'] as const).map((g) => (
            <button
              ...
              style={{
                padding: '0.75rem',
                border: `1px solid ${value.gender === g ? 'var(--color-gold)' : 'var(--color-border)'}`,
                borderRadius: 8,
                background: value.gender === g ? 'rgba(196,168,98,0.12)' : 'rgba(255,255,255,0.03)',
                color: value.gender === g ? 'var(--color-gold)' : 'var(--color-ivory-dim)',
                fontSize: '0.9rem',
                cursor: 'pointer',
              }}
            >

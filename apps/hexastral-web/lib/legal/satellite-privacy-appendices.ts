...
> = {
  ...
  feng: {
    ...
    bullets: [
      ...
      'Paywall: each full site analysis job requires an active universe Pro subscription OR one unconsumed `hexastral_feng_single` one-time purchase (currently from approximately USD $9.99 for a single-floor plan in supported markets). There is no free monthly feng analysis quota. The single purchase is consumed only after a report completes successfully.',
      ...
      'Reports, sites, job status, and chat threads live in `feng_reports`, `feng_sites`, `feng_jobs`, and chat conversation tables under your user id. RevenueCat validates `hexastral_feng_single` and subscription entitlements — we never see your payment card.',

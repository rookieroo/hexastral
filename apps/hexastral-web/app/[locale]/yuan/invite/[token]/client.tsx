...
export function KindredInviteClient({
  ...
  async function handleAccept() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/bonds/invite/${encodeURIComponent(token)}/respond`, {
        method: 'POST',
        headers: {
      ...
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        {stage === 'gate' && (
          <>
            ...
            {!TURNSTILE_SITE_KEY && (
              <button type='button' onClick={() => setStage('form')} style={ctaStyle}>
                {isZh ? '开始 →' : 'Begin →'}
              </button>
            )}
          </>
...
function TurnstileGate({
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <button type='button' onClick={() => onVerified('placeholder-token')} style={ctaStyle}>
        Begin →
      </button>
    </div>
  )

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'en'
          ? `https://hexastral.com/blog/${slug}`
          : `https://hexastral.com/${locale}/blog/${slug}`,
    },
  }
}
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  return (
    <article>
      ...
      <hr
        style={{ border: 'none', borderTop: '1px solid var(--color-border)', margin: '1.75rem 0' }}
      />
      {post.body.split('\n\n').map((para, idx) => (
      ...
      <p style={{ marginTop: '1.5rem' }}>
        <Link
          href='/blog'
          style={{ color: 'var(--color-gold)', fontSize: '0.88rem', textDecoration: 'none' }}
        >
          ← {t('backToBlog')}
        </Link>
      </p>

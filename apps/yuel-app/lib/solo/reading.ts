 ...
 * exactly the numbers + labels the solo /reading screen displays.
 * Ported verbatim from ming-pan-app/lib/reading.ts per ADR-0021 K1 / ADR-0022
 * (pure TS over @zhop/astro-core, no app-specific dependencies).
 *
 ...
 * Deep LLM interpretation arrives via reading-cache (the report chapters).
 */
...
export function parseBirthInput(
  ...
  // Same representative-hour mapping as lib/solo/natal.ts (mirror svc-astro fallback)
  let hour: number

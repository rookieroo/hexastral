 ...
 *   - 出生地 is NOT collected in 时辰 mode. True-solar-time correction only
 *     makes sense on a precise clock, so the birth city lives in the mutually
 *     exclusive exact-time mode (`allowPreciseTime`) — picking a city there
 *     enables 真太阳时 calibration of the hour pillar.
 */

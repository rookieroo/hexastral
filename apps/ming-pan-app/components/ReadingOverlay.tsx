/**
 * ReadingOverlay — in-place ink-bloom that reveals the ReadingReport over the
 * live home screen.
 *
 * Rendered as an absolute child of the home (NOT a pushed route): home stays in
 * the component tree behind, so there is literally no transition to flash. The
 * organic ink mask blooms the paper report in from the CTA origin; outside the
 * shape is transparent → the home shows through. Right-swipe dismisses with a
 * symmetric reverse bloom (the ink collapses back to where it started), then
 * the parent unmounts the overlay.
 */
...
function OverlayInner({ onClose }: { onClose: () => void }) {
  ...
  // Chapter detail navigation lives here (not in ReadingReport) so the
  // overlay's right-swipe can pop a detail before falling through to close.

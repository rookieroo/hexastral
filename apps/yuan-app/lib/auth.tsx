...
export function AuthProvider({ locale, children }: AuthProviderProps) {
  ...
  const refreshProfile = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) console.warn('[Yuán auth] refreshProfile failed', err)
    }
  }, [userId])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (__DEV__) console.error('[Yuán auth] user provisioning failed', err)
        if (!cancelled) setIsLoading(false)
      }
    })()
    return () => {
    ...
    () => ({
      ...
      signOut: async () => {
        ...
        try {
          ...
          if (__DEV__) console.error('[Yuán auth] re-provision after sign-out failed', err)
          setUserId(null)
        }
      },
      resyncCredentials,

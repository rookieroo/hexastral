  ...
  static func strings(for locale: WatchLocale) -> WatchStrings {
    ...
    case .zhHans:
      return WatchStrings(
        ...
        lastSync: "上次同步", credentialLinked: "已关联账号",
        ...
        complicationFaceHint:
          "矩形宜忌槽仅在少数表盘（模块化 / Infograph Modular 等）。更多表盘用圆形、表角或底边一行。圆形/表角：月相 + 干支（节气日显示节气）。底边：宜一词 · 忌一词。对你而言只在 Watch App。",
    ...
    case .zhHant:
      return WatchStrings(
        ...
        lastSync: "上次同步", credentialLinked: "已關聯帳號",
        ...
        complicationFaceHint:
          "矩形宜忌槽僅在少數錶盤（模組化 / Infograph Modular 等）。更多錶盤用圓形、錶角或底邊一行。圓形/錶角：月相 + 干支（節氣日顯示節氣）。底邊：宜一詞 · 忌一詞。對你而言只在 Watch App。",
    ...
    case .ja:
      return WatchStrings(
        ...
        lastSync: "最終同期", credentialLinked: "アカウント連携済み",
        ...
        complicationFaceHint:
          "矩形スロットは少数の文字盤のみ（モジュラー / Infograph Modular など）。多くは円形・コーナー・インラインです。円形/コーナー：月相＋干支（節気の日は節気）。インライン：向く1語 · 避ける1語。「あなたに」は Watch App 内のみ。",
    ...
    case .en:
      return WatchStrings(
        ...
        lastSync: "Last sync", credentialLinked: "Account linked",
        ...
        complicationFaceHint:
          "Rectangular slots appear on few faces (Modular / Infograph Modular). Most faces use circular, corner, or inline. Circular/corner: moon + stem-branch (solar term on term days). Inline: Good one · Avoid one. For you only in the Watch app.",

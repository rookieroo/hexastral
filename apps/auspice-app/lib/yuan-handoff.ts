/**
 * Cycle → Yuán person hand-off.
 *
 * Cycle owns the lightweight 亲友 contact (name + birthday for push). When
 * the user wants a *deep* 合盘 reading on a 亲友, we open Yuán with both
 * birth blocks pre-filled so they don't re-type. App-Store fallback on
 * unverified installs.
 *
 * URL contract (v1) — Yuán must parse the same shape on the receiving end:
 *
 ...
 *     date — Yuán's draft already accepts missing time/gender/city.
 *   - 农历 calendars are NOT shipped: Yuán's draft is solar-only. Lunar
 *     entries on the cycle side are dropped from the hand-off (we expose a
 *     small UI affordance for that case in the caller).
 */

import { Linking } from 'react-native'
import type { CycleBirthInfo } from './birth'
import { FLAGSHIP_LINKS } from './config'
import type { CyclePerson } from './people'

/** Yuán's URL scheme — fixed once at hand-off contract v1. */
const YUAN_SCHEME = 'yuan://'

interface BuildArgs {
  person: CyclePerson
  self: CycleBirthInfo | null
...
export function buildYuanComposeUrl({ person, self, relationshipLabel }: BuildArgs): string {
 ...
 */
export async function openYuanCompose(args: BuildArgs): Promise<boolean> {
  const url = buildYuanComposeUrl(args)
  try {

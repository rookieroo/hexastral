...
export function makeIfCopyForLocale(locale: string): MakeIfCopy {
  ...
  if (locale.startsWith('zh')) {
    return {
      ...
      branches: [
        ...
        {
          label: '做自媒体',
          outcome: 'All in 内容与表达——丙午火运利曝光，乘势而上，可见度与表达有机会打开。',
        },
  ...
  if (locale.startsWith('ja')) {
    return {
      ...
      lockedCta: 'もう一つの人生をひらく · make-if',
      proHint: '過ぎた人生は八字どおりに表示。未来の分岐は解放でひらく。',
      nowLabel: '今',
      explainTitle: 'make-if とは · もしも',
      explainRules: [
        '点線は「もしも」の人生分岐、実線はあなたの本当の命式の主線。',
        'どの重要な選択も人生を分岐させる——中年で本命の調子へ戻る道も、進み続ける道もある。道は違い、行き着く先は己しだい。',
        '解放すると、これらの分岐があなたの八字から生成され、あなただけの「もう一つの人生」になる。',
      ],
      explainUnlock: '自分の make-if をひらく',
      branches: [
        {
          label: 'AIで起業',
        ...
        },
        {
          label: '発信・コンテンツ',
          outcome:
            '内容と発信に全振り——この時代は「見られる」ことに味方し、波に乗れば届く範囲が広がる。',
        },
  ...
  return {
    ...
    branches: [
      ...
      },
      {
        ...
        outcome:
          'Go all-in on content — these times reward being seen; ride the wave and your reach can widen.',
      },
      {
...
export function makeIfInteractiveCopyForLocale(locale: string): MakeIfInteractiveCopy {
  ...
  if (locale.startsWith('zh')) {
    return {
      screenTitle: '假如人生',
      tapHint: '点主线上任一节点，假设一个选择，探索另一种人生。',
      forkTitle: (age, isPast) => (isPast ? `假如当年 ${age} 岁…` : `假如在 ${age} 岁…`),
      eventChips: presetChipsForLocale('zh-Hans'),
      eventPlaceholder: '自定义一个事件…',
      submit: '探索',
      generating: '生成中…',
      failedRetry: '生成失败 · 点此重试',
      limited: '今日生成次数已用完',
      disclaimerTitle: '开始前请知悉',
      disclaimerBody:
        'make-if 是基于你八字的对照参考，并不是预言，也不替你做决定。过去无法改变，所以是「假如」；改变现在或未来的假设只用于自我探索。现实中的每一个选择与后果，都由你自己独立判断、自行承担。',
      disclaimerAck: '我已理解，继续',
      footer: '仅供参考与省思，非预测或建议；一切现实决定由你自行承担。',
      unlockTitle: 'What-If · 假如人生',
      unlockBody: '解锁后，可在你真实人生线的任一节点假设一个选择，由你的八字对照探索另一种人生。',
      unlockCta: '解锁假如人生',
  ...
  if (locale.startsWith('ja')) {
    return {
      ...
      limited: '本日の利用上限に達しました',
      disclaimerTitle: 'はじめる前に',
      disclaimerBody:
        'make-if は八字に基づく文化的な参照であり、予言ではなく、あなたの代わりに決定するものでもありません。過去は変えられないため「もしも」です。現在や未来の仮定は自己探索のためのものです。現実のすべての選択と結果は、あなた自身の判断と責任で行われます。',
      disclaimerAck: '理解しました。続ける',
      footer:
        '参考と振り返りのためのもので、予測や助言ではありません。現実の決定はすべてご自身の責任です。',
      unlockTitle: 'What-If · もしもの人生',
      unlockBody:
        '解放すると、あなたの本当の人生線の任意の節目で選択を仮定し、八字からもう一つの人生を描けます。',
      unlockCta: 'もしもの人生をひらく',

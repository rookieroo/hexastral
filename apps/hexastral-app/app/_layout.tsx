...
import { setFunnelLocale, setFunnelUserId } from '@/lib/analytics'
...
async function handleDDLFromURL(url: string | null): Promise<void> {
  ...
  // Also capture satellite → flagship attribution if this is an `onboard` URL.
  // Independent of DDL: a satellite may send `?from=&signal=` without `?ddl=`.
  await captureOnboardAttribution(url)
}
...
function RootLayoutInner() {
  ...
  // Keep funnel emitter's identity/locale resolvers in sync with React state.
  useEffect(() => {
    setFunnelUserId(userId ?? null)
  }, [userId])
  useEffect(() => {
    setFunnelLocale(locale ?? null)
  }, [locale])
  ...
  return (
    <ThemeProvider value={NAV_THEME[colorScheme ?? 'light']}>
      ...
      <CoreUIProvider brand='hexastral' mode={colorScheme === 'dark' ? 'dark' : 'light'}>

...
function isNatalGeJu(v: unknown): v is NatalGeJu {
  ...
  if (typeof v.favorableElement !== 'string' || typeof v.unfavorableElement !== 'string')
    return false
...
function toFateNatalChart(chart: unknown): FateNatalChart | null {
  ...
  const dm =
    typeof chart.dayMaster === 'string' && chart.dayMaster.length > 0 ? chart.dayMaster : day.stem
  const shishen =
    chart.shishen != null && isFourPillarsShiShen(chart.shishen) ? chart.shishen : undefined
...
export default function NatalScreen() {
  ...
  const interpretation: NatalInterpretation | null =
    interpRaw && typeof interpRaw === 'object' && interpRaw !== null && 'overview' in interpRaw
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        ...
        {chart.shishen ? (
          <NatalShiShenGrid shishen={chart.shishen} colors={baziColors} t={t} />
        ) : null}
        {interpretation ? (

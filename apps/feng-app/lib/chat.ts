/**
 * Fēng reading chat — HMAC-signed adapter over POST/GET /api/chat.
 *
 ...
 * envelope's `error.message` so the screen can react (Fēng has no paywall screen
 * yet, so it surfaces a Pro notice instead).
 *
 * Conversations are scoped server-side by `X-Target-App: feng`. The `'feng'`
 * reading-context query keys on the fengReports id.
 */

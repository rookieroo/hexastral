...
export function SatelliteHistoryList(props: SatelliteHistoryListProps) {
  ...
  if (loading) {
    if (listChrome) {
      return (
        <ScrollView contentContainerStyle={[styles.chromeContent, styles.chromeContentGrow]} keyboardShouldPersistTaps='handled'>
          {props.listHeader}
  ...
  if (error) {
    ...
    const errorBody = (
      <View style={[styles.center, listChrome ? styles.chromeBodyMinHeight : undefined]}>
        ...
        {!authError ? (
          <Pressable style={[styles.retry, { borderColor: colors.separator }]} onPress={() => void loadFirst()}>
  ...
  return (
    <FlatList
      ...
      ListHeaderComponent={
        props.listHeader !== undefined ? <View collapsable={false}>{props.listHeader}</View> : undefined
      }
      ListFooterComponent={
        showListFooter ? (
      ...
      renderItem={({ item }) =>
        props.renderItem ? (
          ...
          <View style={[styles.row, { borderColor: colors.separator, backgroundColor: colors.card }]}>
            <Text style={[styles.primary, { color: colors.text }]}>{item.readingType}</Text>
            <Text style={[styles.secondary, { color: colors.secondary }]}>{new Date(item.createdAt).toLocaleString()}</Text>
          </View>
        )
      }
      ListEmptyComponent={<Text style={[styles.empty, { color: colors.secondary }]}>{props.emptyText ?? 'No readings yet.'}</Text>}
    />
  )
}

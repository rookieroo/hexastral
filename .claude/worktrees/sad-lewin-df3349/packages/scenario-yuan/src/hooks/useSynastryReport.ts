...
export function useSynastryReport(bondId: string | null): UseSynastryReportResult {
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await client.api.bonds[':id'].$get({ param: { id: bondId } })
      if (res.status === 202) {
      ...
      if (!res.ok) {
        throw new Error(`Bond detail fetch failed: ${res.status}`)
      }
      const json = (await res.json()) as ApiResponse<BondDetailData>
      setDetail(json.data)

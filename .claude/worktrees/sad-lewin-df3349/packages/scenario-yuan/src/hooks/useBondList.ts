...
export function useBondList(options: UseBondListOptions = {}): UseBondListResult {
  ...
  const refetch = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await client.api.bonds.$get()
      if (!res.ok) throw new Error(`Bonds fetch failed: ${res.status}`)
      const json = (await res.json()) as ApiResponse<{ bonds: BondData[] }>
      setAllBonds(json.data.bonds)
    } catch (err) {
  ...
  const filtered = allBonds.filter((b) => {
    ...
    if (options.relationshipType && !matchesRelationshipType(b.relationshipLabel, options.relationshipType)) {
      return false
    }
    return true
  })

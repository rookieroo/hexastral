...
- `apps/yuan-app` *(planned, Phase B)* — standalone Expo build
- `apps/hexastral-app` *(migration planned, see [MIGRATION.md](./MIGRATION.md))* — the 缘 Bonds tab
...
```tsx
import { useBondInvitation, useBondList, useSynastryReport } from '@zhop/scenario-yuan'
...
├── context.tsx               <YuanClientProvider> + useYuanClient hook
├── hooks/
...
| `YuanClientProvider` | ✅ Implemented |

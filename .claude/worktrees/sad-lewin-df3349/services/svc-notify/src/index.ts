...
app.get('/expo-push/locale/:userId', async (c) => {
  ...
  const record = await c.env.EXPO_PUSH_TOKENS.get(`pushtoken:${userId}`, 'json') as PushTokenRecord | null
...
app.post('/expo-push/notify-user', async (c) => {
  ...
  const record = await c.env.EXPO_PUSH_TOKENS.get(`pushtoken:${body.userId}`, 'json') as PushTokenRecord | null
...
function getDailyFallbackText(locale: string): { title: string; body: string } {
  const pool: Record<string, { title: string; body: string }[]> = {
    zh: [
      { title: '今日运势已就绪', body: '你的星盘已为今日运算完毕，开启应用查看专属解读。' },
    ],
    'zh-Hant': [
      { title: '今日運勢已就緒', body: '你的星盤已為今日運算完畢，開啟應用查看專屬解讀。' },
    ],
    en: [
      { title: "Today's reading is ready", body: 'Open HexAstral to see your daily insight.' },
    ],
    ko: [{ title: '오늘의 운세가 준비되었습니다', body: '앱을 열어 오늘의 사주를 확인해 보세요.' }],
    ja: [{ title: '本日の運勢が届きました', body: 'アプリを開いて運勢をご確認ください。' }],
    de: [{ title: 'Ihre Tagesvorhersage ist bereit', body: 'Öffnen Sie HexAstral für Ihre tagesaktuelle Deutung.' }],
    es: [{ title: 'Tu lectura de hoy está lista', body: 'Abre HexAstral para ver tu interpretación diaria.' }],
  ...
  return candidates[0] ?? { title: 'Your reading awaits', body: 'Open HexAstral for today\'s insight.' }
}
...
async function runHourlyFortunePush(env: Env): Promise<void> {
  ...
  for (const tz of matchingZones) {
    ...
    while (cursor !== null) {
      ...
      for (let i = 0; i < batch.length; i += 100) {
        ...
        await env.DAILY_FORTUNE_QUEUE.sendBatch(
          chunk.map((msg) => ({ body: msg })),
        )
      }
  ...
  logger.info('Hourly fortune push complete', { timezonesMatched: matchingZones.length, totalEnqueued, totalPages })
}
...
async function fetchD1WithRetry(fetcher: Fetcher, url: string, options: any, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    ...
    await new Promise(r => setTimeout(r, Math.random() * 1000 * Math.pow(2, i)));
  }
}
...
async function queue(
  batch: MessageBatch<DailyFortuneMessage>,
  env: Env,
): Promise<void> {
  ...
  const pushTasks = batch.messages.map(async (msg) => {
    ...
    if (almanacRes && almanacRes.ok) {
      const body = (await almanacRes.json().catch(() => null)) as
        | { data: AlmanacRow | null }
        | null
    ...
    try {
      ...
      if (invalidTokens.length > 0) {
        ...
        if (usedFallback) fallback++ ; else sent++
      }
  ...
  logger.info('Queue batch complete', { total, sent, fallback, failed, invalidTokens: invalidTokenCount })
}

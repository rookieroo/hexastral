 ...
 *
 * 模型策略:
 *   主力: Gemini 3.1 Pro (gemini-3.1-pro-preview) — 长上下文推理，玄学语料强
 *   备用: DeepSeek-R1 (deepseek-reasoner) — Chain-of-Thought，复杂宫位推演
 *
 * 备用条件: Gemini API 故障/超时时自动切换，日志记录所用模型
 */
...
import { fateOutputSchema, toJsonSchema, type FateOutput } from '../../schemas'
...
export async function generateFateReport(
  ...
  // 3-tier fallback: Gemini (responseSchema) → DeepSeek-V3 (json_object) → Workers AI

...
import { callGeminiVision, callGeminiVisionStructured } from '../lib/gemini'
...
export async function extractFaceFeatures(
  ...
  return callGeminiVisionStructured<FaceFeatures>(
    apiKey,
    FACE_FEATURES_SYSTEM_PROMPT,
    '请按要求提取面相特征，只输出 JSON，不要任何额外文字。',
    imageBase64,
    FACE_FEATURES_SCHEMA,
    mimeType
  )
}
...
export async function describePhysiognomy(
  ...
  try {
    const description = await callGeminiVision(apiKey, systemPrompt, userPrompt, photoBase64)
    ...
    `**输出格式要求**（严格按此JSON结构输出）：
{
  ...
  "specialNotes": "特别注意事项或开运建议 2-3 句话"
    ...
    solarBirthDate ? buildAgeLanguageBlock(solarBirthDate, language) : buildLanguageBlock(language, 'physiognomy'),
  ].join('\n')

...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'en' ? `https://hexastral.com/palace/${slug}` : `https://hexastral.com/${locale}/palace/${slug}`,
    },
  }
}
...
export default async function PalaceReferencePage({ params }: Props) {
  ...
  return (
    <article>
      ...
      />
      <header>
        <p style={{ letterSpacing: '0.2em', color: 'var(--color-gold)', fontSize: '0.73rem', marginBottom: '0.35rem' }}>
          STAR PALACE · 宫位
        </p>
      ...
      <Link href='/tools/palace-chart' style={{ color: 'var(--color-gold)', fontSize: '0.85rem', display: 'inline-block', marginTop: '1rem' }}>

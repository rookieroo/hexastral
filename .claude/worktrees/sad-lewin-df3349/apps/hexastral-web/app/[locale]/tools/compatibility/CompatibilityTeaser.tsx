...
import { computeCompatibilityPreview, type CompatibilityPreviewResult } from '@/app/actions/chart'
  ...
  if (result) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        ...
        >
          <p style={{ fontSize: '0.8rem', color: 'var(--color-gold)', margin: '0 0 0.35rem', letterSpacing: '0.12em' }}>
        ...
        <ul style={{ paddingLeft: '1.1rem', margin: 0, color: 'var(--color-ivory-dim)', lineHeight: 1.7 }}>
      ...
      {step === 'a' ? (
        ...
        <>
          ...
          <div style={{ alignSelf: 'stretch' }}>
            <TurnstileWidget key={`compat-${tsKey}`} action='growth-compat-teaser' onToken={setTurnstileToken} />
          </div>
          {error ? (

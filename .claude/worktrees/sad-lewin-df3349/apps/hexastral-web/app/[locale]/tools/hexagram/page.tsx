...
export default function HexagramIndexPage() {
  return (
    <>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 400, marginTop: 0 }}>I Ching · 周易 hexagram index</h1>
      <p style={{ color: 'var(--color-ivory-dim)', marginBottom: '1.25rem', lineHeight: 1.65 }}>
        Each entry links to commentary suited for SERP snippets. Shake‑to‑oracle UX ships in HexAstral /
        forthcoming CoinCast.
      </p>
      <div
        style={{
          display: 'grid',
      ...
      >
        {HEXAGRAM_LIST.map((h) => (
          ...
          >
            <div style={{ color: 'var(--color-gold)', fontVariantNumeric: 'tabular-nums' }}>#{h.number}</div>
            <div style={{ fontSize: '1.1rem' }}>
              {h.symbol} {h.name}
            </div>
            <div style={{ color: 'var(--color-ivory-muted)', textTransform: 'capitalize' }}>{h.pinyin}</div>
          </Link>
        ))}
      </div>
      <div style={{ marginTop: '2rem' }}>
        <DownloadCTA headline='Shake the hexagram in app' sub='Coins, lines, AI judgment — Liu Yao 六爻 in HexAstral.' />
      </div>
    </>
  )
}

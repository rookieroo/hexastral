...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical:
        locale === 'en' ? 'https://hexastral.com/tools/dream' : `https://hexastral.com/${locale}/tools/dream`,
    },
  }
}
...
export default function DreamTeaserToolPage() {
  return (
    <>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 400, marginTop: 0 }}>DreamInterpreter · 梦境</h1>
      <p style={{ color: 'var(--color-ivory-dim)', lineHeight: 1.65 }}>
        We&apos;re carving <strong>DreamOracle</strong> — text-first dream journaling with Zhou Gong motif vocabulary (
        <em>周公解梦</em>) plus Jungian scaffolding for English-native readability.
      </p>
      <div
        style={{
          padding: '1rem',
      ...
      >
        Web upload is intentionally paused while we tighten privacy narratives. Subscribe to HexAstral on iOS today for
        I Ching · destiny flows; DreamOracle inherits the same account graph when it ships.
      </div>
      <DownloadCTA

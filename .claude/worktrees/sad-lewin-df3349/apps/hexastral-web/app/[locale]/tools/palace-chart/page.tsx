...
export default async function PalaceChartIntroPage({ params }: { params: Promise<{ locale: string }> }) {
  await params

  return (
    <>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 400, marginTop: 0 }}>Zi Wei palace map · 紫微十二宫</h1>
      <p style={{ color: 'var(--color-ivory-dim)', lineHeight: 1.65 }}>
        Zi Wei Dou Shu fans often compare this to twelve astrological houses: each palace becomes a semantic container
        for stars, transformers (四化 sì huà), and timing overlays like decade cycles (<em>大运 dà yùn</em>).
      </p>
      <p style={{ fontSize: '0.82rem', color: 'var(--color-gold)' }}>Starter trio for discovery</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
        {PALACE_SLUGS.filter((slug) => ['life-palace', 'career', 'spouse'].includes(slug)).map((slug) => (
          <Link
            key={slug}
            href={`/palace/${slug}`}
            prefetch={false}
      ...
      />
      <DownloadCTA headline='Charts already plotting in HexAstral flagship' compact targetApp='hexastral' />
    </>
  )
}

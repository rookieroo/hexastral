...
export default async function DayMasterReferencePage({ params }: Props) {
  ...
  return (
    <article>
      <JsonLd json={faqJson} />
      <header>
        <p style={{ fontSize: '0.75rem', letterSpacing: '0.2em', color: 'var(--color-gold)' }}>DAY MASTER · 日元</p>
        ...
        <p style={{ color: 'var(--color-gold)', fontSize: '0.92rem' }}>Element focus: {dm.element}</p>
      </header>
      <p style={{ marginTop: '1.25rem', lineHeight: 1.75, color: 'var(--color-ivory)', fontSize: '0.96rem' }}>
        {dm.blurb}
      </p>
      ...
      <DownloadCTA headline='HexAstral dual-chart engine' sub='Ba Zi × Zi Wei × Bonds — already live on iOS.' compact />
    </article>
  )
}

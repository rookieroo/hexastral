...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: locale === 'en' ? 'https://hexastral.com/blog' : `https://hexastral.com/${locale}/blog`,
    },
  }
}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {BLOG_POSTS.map((post) => (
          ...
          >
            ...
            <h2 style={{ margin: '0.35rem 0', fontWeight: 500, fontSize: '1.12rem', color: 'var(--color-ivory)' }}>
              {post.title}
            </h2>
            <p style={{ margin: 0, color: 'var(--color-ivory-dim)', fontSize: '0.88rem' }}>{post.description}</p>
            <span style={{ display: 'inline-block', marginTop: '0.55rem', color: 'var(--color-gold)', fontSize: '0.82rem' }}>
              {t('readArticle')} →
            </span>
          </Link>
        ))}

...
import { getPublicPlainIntroHeading } from '@/lib/publicProfilePlainIntroHeading'
import { dedupePublicSignature } from '@/lib/publicProfileSignature'
import {
  isEnglishLocale,
  PUBLIC_PROFILE_EN_CHART_NOTE,
} from '@/lib/publicProfileEnChartNote'
  ...
  const sig =
    vis?.signature && user.fateSignature?.trim()
      ...
      : derivedSig?.signature.trim()
        ? derivedSig.signature.trim()
        : null
  const baseDesc = `View ${displayName}'s birth chart on HexAstral — AI-powered destiny charts & star palace readings.`
  const ogDesc = sig ? `${displayName} · ${sig}` : `${displayName}'s cosmic blueprint, powered by AI.`
  return {
...
export default async function PublicProfilePage({ params }: PublicProfilePageProps) {
  ...
  const joinYear =
    showBasic && user.createdAt ? new Date(user.createdAt).getFullYear() : null
  ...
  const { headline: signatureHeadline, sub: signatureSub } = dedupePublicSignature(rawHeadline, rawSub)
  ...
  const plainIntro =
    showPlainIntro && typeof user.plainIntroExcerpt === 'string'
      ...
      : ''
  const plainIntroHeading = plainIntro ? await getPublicPlainIntroHeading(user.locale) : ''
      ...
      >
        ...
        >
          ...
          {showBasic && joinYear != null ? (
            ...
            >
              {user.totalReadings ? `${user.totalReadings} readings · ` : ''}
              On HexAstral since {joinYear}
            </p>
          ) : null}
        </div>

        {/* Fate signature — gated by visibility.signature */}
        {showSignature && (signatureHeadline || signatureSub) ? (
          <div
            style={{
        ...
        {/* Ba Zi four pillars — when `/chart` returns natal (visibility.bazi) */}
        {showBirth && natalPillars ? <PublicNatalPillars pillars={natalPillars} /> : null}
        ...
        >
          ...
          >
            AI-powered Ba Zi & Purple Star astrology — free on iOS
          </p>

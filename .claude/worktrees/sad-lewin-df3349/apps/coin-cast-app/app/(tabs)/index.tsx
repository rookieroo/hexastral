...
import { CONTAINER_SHAKE_DURATION_MS, PHYSICS_COMMIT_FALLBACK_MS } from '@/components/casting-scene/constants'
...
import { useSatelliteI18n, type SatelliteLocaleKey } from '@/lib/i18n'
...
export default function CoinCastHomeScreen() {
  ...
  const runEntropyCommit = useCallback(
    async (q: string, lineCount: number) => {
      ...
      const result =
        hashHex === 'fallback'
          ? coinsFromSeedSyncFallback(snap, lineCount)
          : coinsFromSeed(seed)
  ...
  const castingBackdrop = isDark ? '#2a231c' : '#2e261d'
  ...
  return (
    ...
    >
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <View style={styles.inner}>
            ...
            {lineRows.length > 0 ? (
              <View
                style={[
                  styles.yaoOverlay,
                  {
                    backgroundColor: isDark ? 'rgba(20,18,14,0.92)' : 'rgba(252,249,244,0.94)',
                  },
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    {
                      color:
                        loading || completed >= 6 ? colors.dim : colors.secondary,
                    },
                  ]}
                >
          ...
          {blockIosDevAccelShake && motionEnabled ? (
            <Text style={[styles.devShakeHint, { color: colors.dim }]}>{t('homeDevShakeHint')}</Text>
          ) : null}
        </View>
      </TouchableWithoutFeedback>
      {breathingOverlayVisible ? (
        ...
        >
          <Text style={[styles.breathOverlayText, { color: colors.text }]}>{t('breathingOverlayText')}</Text>
          <Animated.View
            style={[

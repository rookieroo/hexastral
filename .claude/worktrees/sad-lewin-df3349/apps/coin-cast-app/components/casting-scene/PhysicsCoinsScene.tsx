...
function finalizeCoinsOnTable(bodies: CANNON.Body[], result: YaoResult, _impulseSeed: number): void {
  for (let i = 0; i < 3; i++) {
...
function spawnCoinsInVessel(bodies: CANNON.Body[], impulseSeed: number, tossRevision: number): void {
 ...
 */
function applyHandsOpenRelease(bodies: CANNON.Body[], impulseSeed: number, tossRevision: number): void {
...
export function PhysicsCoinsScene(props: PhysicsCoinsSceneProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (phaseRef.current === 'active' && bodies.length === 3) {
      if (tossSubPhaseRef.current === 'container_shake' && frameT >= containerShakeEndMsRef.current) {
        clearArenaWalls()
    ...
    if (phaseRef.current === 'active' && tossSubPhaseRef.current === 'released' && bodies.length === 3) {
      const linearCalm = bodies.every(
        (b) => b.velocity.lengthSquared() < RIM_STUCK_LINEAR_SQ
      )
      const anyRimLow = bodies.some(
        (b) => bodyOnRim(b) && b.position.y < COIN_RADIUS + 0.055
      )
    ...
    if (phaseRef.current === 'active' && drive && drive.mag > 1.05) {
      ...
      for (const b of bodies) {
        b.wakeUp()
        b.applyImpulse(
          new CANNON.Vec3(drive.x * s, drive.y * s * 0.22, drive.z * s),
          b.position
        )
      }
    }
  ...
  return (
    <>
      ...
      <hemisphereLight args={['#d8cfc4', backdrop, 0.48]} position={[0, 8, 0]} />

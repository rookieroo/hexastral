...
/** POST / — 命格推演 + AI 解读 */
export const natalRoutes = new Hono<AppEnv>().post('/', async (c) => {
  ...
  const requestUser = await db.select().from(users).where(eq(users.id, requestUserId)).get()
  const isRequestorPro = isProUser(requestUser)

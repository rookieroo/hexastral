...
export const signalHistoryRoutes = new Hono<AppEnv>().get('/', async (c) => {
  ...
  if (!parsed.success) {
    throw new HTTPException(400, { message: 'Invalid query: days must be 1–90' })
  }
...
export const signalItemRoutes = new Hono<AppEnv>()
  .get('/:signalId', async (c) => {
    ...
    if (!signalId) {
      throw new HTTPException(400, { message: 'Missing signal id' })
    }
    const db = c.get('db')
    ...
    if (!parsed.success) {
      ...
      throw new HTTPException(500, { message: 'Invalid signal payload' })
    }
  ...
  .delete('/:signalId', async (c) => {
    ...
    await db.delete(dailySignals).where(and(eq(dailySignals.id, signalId), eq(dailySignals.userId, userId)))

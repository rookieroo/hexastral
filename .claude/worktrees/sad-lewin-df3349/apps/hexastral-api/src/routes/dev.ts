 ...
 *
 * Available actions:
 *   POST   /api/dev/set-subscription  — override subscriptionStatus for the authed user
 ...
 *
 * Directly writes subscriptionStatus to D1 so the app reflects Pro/Free
 * without needing a real RevenueCat sandbox purchase.
 */
devRoutes.post(
  '/set-subscription',
  ...
  async (c) => {
    ...
    return c.json({ ok: true, subscriptionStatus: status })
  }
)
 ...
 *
 * Account identity (id, email, apple/google sub, subscription_status) is preserved

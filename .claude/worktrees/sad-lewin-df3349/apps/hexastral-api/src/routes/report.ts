...
function assertChapterAccess(slug: ChapterSlug, isPro: boolean) {
  if (isPro) return
  if (!FREE_CHAPTERS.has(slug)) {
    throw new HTTPException(403, {
      message: 'Subscription required for this chapter',
    })
  }
}
...
export const reportManifestRoutes = new Hono<AppEnv>().get('/', async (c) => {
  ...
  const chapters = CHAPTER_SLUGS.map((slug) => {
    ...
    return {
      ...
      isFree: FREE_CHAPTERS.has(slug),
      accessible: FREE_CHAPTERS.has(slug) || isPro,
      hasCurrent: !!cur,
      generatedAt,
      generationBatchId,
  ...
  if (missing.length > 0) {
    c.executionCtx.waitUntil(
      Promise.allSettled(
        missing.map((ch) => generateChapter({ c, userId, slug: ch.slug }))
      ).then((results) => {
...
export const reportChapterRoutes = new Hono<AppEnv>()
  .get('/:slug', async (c) => {
    ...
    const user = await db.query.users.findFirst({ where: eq(users.id, userId) })
    assertChapterAccess(slug, isProUser(user))
    ...
    if (!isProUser(user) && cur && cur.chartHash !== chartHash) {
      return c.json(
        {
          error:

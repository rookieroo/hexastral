...
/** Free tier may read these. Other chapters are Pro-only. */
export const FREE_CHAPTERS: ReadonlySet<ChapterSlug> = new Set([
  'ch1_personality',
  'ch2_dimensions_static',
  'ch3_stellar',
])
...
export async function loadChartContext(
  ...
  const currentDayun =
    natal.chartType === 'natal' ? computeCurrentDayun(natal.solarDate, natal.gender, today) : 'unknown'

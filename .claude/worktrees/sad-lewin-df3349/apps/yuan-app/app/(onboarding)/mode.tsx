...
function OverlappingCircles({ size = 100 }: { size?: number }) {
  ...
  return (
    <Svg width={size} height={size * 0.7}>
      <Circle cx={size / 2 - offset} cy={size * 0.35} r={r} stroke={ink.gold} strokeWidth={1.2} fill="none" />
      <Circle cx={size / 2 + offset} cy={size * 0.35} r={r} stroke={ink.gold} strokeWidth={1.2} fill="none" />
    </Svg>
  )
}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: yuanLight.bg }}>
      ...
      >
        ...
        <View style={{ alignItems: 'center', marginTop: yuanSpacing.xxl, marginBottom: yuanSpacing.xl }}>
        ...
        >
          <Text style={[yuanType.heading, { color: yuanLight.text }]}>{t(locale, 'mode.invite')}</Text>
        ...
        >
          <Text style={[yuanType.caption, { color: yuanLight.accent, textDecorationLine: 'underline' }]}>
            {t(locale, 'mode.fill')}
          </Text>
        </Pressable>

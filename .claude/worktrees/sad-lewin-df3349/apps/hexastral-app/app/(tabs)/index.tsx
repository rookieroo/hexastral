...
import { FateActionDeck, type OracleCastHint } from '@/components/fate/QuickActions'
import {
...
import { useFreeQuotaQuery, useQuota, useQuotaQuery, type QuotaStatus } from '@/lib/ux/useQuota'
...
export default function HomeScreen() {
  ...
  const oracleCastHint = useMemo((): OracleCastHint | null | undefined => {
    ...
    if (isPro) {
      ...
      return {
        type: 'pro',
        monthLeft: q.divination.remaining,
        todayLeft: q.divinationToday.remaining,
      }
    }
    const rem = freeQuotaQuery.data?.divination.remaining
    if (typeof rem === 'number') return { type: 'free_remaining', n: rem }
    return undefined
  }, [userId, isPro, quotaQuery.data, freeQuotaQuery.data?.divination.remaining])
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: ios.bg }}>
      ...
      >
        ...
        {/* ── Shortcuts: inset card (hero above stays borderless) ─ */}
        <FateHomeInsetCard marginTop={32}>
          <FateActionDeck
            isPro={isPro}
            onResonance={() => router.push('/(bonds)/bond-create?mode=resonance')}
            onAsk={() => router.push('/(tabs)/report?from=fate' as never)}
            onCast={() => router.push('/(tabs)/yiching?from=fate' as never)}
            onSolo={() => {
              if (isPro) router.push('/(bonds)/bond-create?mode=solo')
              else showPaywallModal()
            }}

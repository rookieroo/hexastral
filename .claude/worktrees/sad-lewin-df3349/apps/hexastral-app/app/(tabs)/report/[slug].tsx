  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: ios.bg }]} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {versionOverride ? (
          <Pressable
            ...
            style={[styles.versionBanner, { backgroundColor: ios.card, borderColor: colors.border }]}
          >
        ...
        ) : error instanceof ReportChapterError &&
          error.code === 'REPORT_REGEN_REQUIRES_PRO' ? (
          <View
            style={[styles.lockedCard, { backgroundColor: ios.card, borderColor: colors.border }]}
          >
            <Lock size={28} color={ios.dim} />
          ...
          <View>
            ...
            {slug === 'ch4_timeline' && content.currentDayunOverview ? (
              <View style={styles.sectionsBlock}>
                <Text style={[styles.sectionLabel, { color: ios.dim }]}>{t('report_sections')}</Text>
            ...
            {accessible ? (
              <AskHexAstralCTA
                ...
                onPress={() =>
                  router.push(
                    `/detail/chat/report/${effectiveUserId}-${slug}` as never
                  )
                }
              />
            ) : null}
      ...
      <HistoryDrawer
        ...
        onSelect={(item, label) => {
          setVersionOverride({ contentJson: item.contentJson, generatedAt: item.generatedAt, label })

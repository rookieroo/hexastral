...
export function FateRecentStrip({ userId }: { userId: string | null | undefined }) {
  ...
  const { chips, historyScope } = useMemo(() => {
    ...
    if (yi.length > 0) {
      return {
        chips: yi.map((r) => ({
          ...
          subtitle:
            r.method === 'liuyao' || r.method === 'meihua'
              ? t(`history_method_${r.method}` as Parameters<typeof t>[0])
    ...
    if (sliced.length === 0) {
      return { chips: [] as { key: string; title: string; subtitle: string }[], historyScope: 'daily' as const }
    }
    return {
      chips: sliced.map((it) => ({
        key: it.signalId,

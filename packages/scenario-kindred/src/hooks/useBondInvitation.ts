 ...
 * @zhop/hexastral-client RPC client wired via <KindredClientProvider>.
 *
...
export function useBondInvitation(token?: string): UseBondInvitationResult {
    ...
    ;(async () => {
      try {
        const data = await unwrap<InvitationInfo>(
          await kindredBonds(client).invite[':token'].info.$get({ param: { token } })
        )
        if (!cancelled) {
          setInvitation(data)
          setIsLoading(false)
  ...
  const create = useCallback(
    async (input: ResonanceInviteInput): Promise<ResonanceInviteResult> => {
      return unwrap<ResonanceInviteResult>(await kindredBonds(client).invite.$post({ json: input }))
    },
    [client]
  )
  ...
  const respond = useCallback(
    async (tk: string, input: RespondInput): Promise<RespondResult> => {
      return unwrap<RespondResult>(
        await kindredBonds(client).invite[':token'].respond.$post({
          param: { token: tk },
          json: input,
        })
      )

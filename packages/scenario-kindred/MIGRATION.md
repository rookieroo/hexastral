...
consume `@zhop/scenario-kindred` instead of the legacy `lib/domain/bonds.ts`
...
The migration becomes valuable once `apps/kindred-app/` is created in Phase B —
...
### Step 1 — Wire KindredClientProvider into hexastral-app
...
<KindredClientProvider client={yuanClient}>
  {/* existing tabs */}
...
// After
import { useBondList } from '@zhop/scenario-kindred'
...
|---|---|---|
| `BondData` | `BondData` (from `@zhop/scenario-kindred`) | identical |
...
The "not yet built" hooks are scoped for Phase B when `apps/kindred-app/` lands —

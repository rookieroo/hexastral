...
import { analyzeShiShenStrength, BRANCH_HIDDEN_STEMS, countShiShen } from './shishen'
...
const ZHUANWANG_GEJU: readonly GeJuType[] = ['曲直格', '炎上格', '稼穑格', '从革格', '润下格']

/** 生我的五行（印星）。 */
function elementGenerating(w: WuXing): WuXing {
  return (Object.entries(WUXING_GENERATE).find(([, gen]) => gen === w)?.[0] as WuXing) ?? w
}

/** 克我的五行（官杀）。 */
function elementOvercoming(w: WuXing): WuXing {
  return (Object.entries(WUXING_OVERCOME).find(([, ov]) => ov === w)?.[0] as WuXing) ?? w
}

/**
 * 推荐用神（喜用神）。
 *
 * 专旺格 / 从格 的取用与正格**相反** —— 顺其旺势而非制衡，所以必须按格局类型分流；
 * 只看日主强弱会把旺神当忌神（专旺）、或用印生身而破格（从格）。
 * - 专旺格：用神为旺神本气，忌克旺神之官杀。
 * - 从格：用神为所从之神（财/杀/食伤），忌印绶生身破格。
 * - 正格 / 普通格：按日主强弱平衡取用。
 */
export function recommendFavorableElement(
  ...
  // 专旺格（一气独旺）：使旺者更旺，忌克旺神。
  if (ZHUANWANG_GEJU.includes(geju)) {
    return { favorable: dayWuXing, unfavorable: elementOvercoming(dayWuXing) }
  }

  // 从格（日主极弱，从其旺势）：用所从之神，忌印绶（生身破从）。
  switch (geju) {
    case '从财格':
      return { favorable: WUXING_OVERCOME[dayWuXing], unfavorable: elementGenerating(dayWuXing) }
    case '从杀格':
      return { favorable: elementOvercoming(dayWuXing), unfavorable: elementGenerating(dayWuXing) }
    case '从儿格':
      return { favorable: WUXING_GENERATE[dayWuXing], unfavorable: elementGenerating(dayWuXing) }
    case '从势格':
      return { favorable: WUXING_OVERCOME[dayWuXing], unfavorable: elementGenerating(dayWuXing) }
    default:
      break
  ...
  if (strength === '极强' || strength === '偏强') {
    // 日主强：喜泄耗（食伤），忌同类。
    return { favorable: WUXING_GENERATE[dayWuXing], unfavorable: dayWuXing }
  }
  if (strength === '极弱' || strength === '偏弱') {
    // 日主弱：喜生扶（印），忌克我。
    return { favorable: elementGenerating(dayWuXing), unfavorable: WUXING_OVERCOME[dayWuXing] }
  }
  // 中和：默认以财为用。
  return { favorable: WUXING_OVERCOME[dayWuXing], unfavorable: dayWuXing }
}
...
export function analyzeGeJu(pillars: FourPillars, shishen: FourPillarsShiShen): GeJuAnalysis {
  ...
  // 3. 推荐用神（专旺/从格 按格局分流，正格按强弱）
  const { favorable, unfavorable } = recommendFavorableElement(pillars.day.stem, strength, primary)

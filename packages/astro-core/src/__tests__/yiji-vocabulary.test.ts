...
describe('yiji-vocabulary', () => {
  ...
  test('hot-word aliases resolve without mutating OFFICER_YIJI', () => {
    ...
    expect(resolveYijiSearchVerbs('换工作')?.verbs).toEqual(['见贵', '求财'])
    expect(resolveYijiSearchVerbs('求职')?.verbs).toEqual(['见贵', '求财'])
    expect(resolveYijiSearchVerbs('装修')?.verbs).toEqual(['修造'])
    expect(resolveYijiSearchVerbs('买车')?.verbs).toEqual(['交易'])
    expect(resolveYijiSearchVerbs('new job')?.verbs).toEqual(['见贵', '求财'])

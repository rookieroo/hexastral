 ...
 * 命格预设人格画像 — 确定性命理词库
 *
 * 客户端（移动端）副本：与 services/svc-astro/src/lib/personality-presets.ts 及
 * apps/hexastral-api/src/lib/personality-presets.ts 同源（内容逐字对齐）。服务端
 * 在 LLM 失败时返回这套兜底；客户端 free 档的 teaser 同样取自这里，保证不调用 LLM
 * 时用户也能拿到命盘相关、而非泛化的内容（teaser-narrator.ts 在此之上拼装成文）。
 * 改动任意一处时，三处需保持一致。
 *
 * 用途：命格派生的确定性人格内容（永不出现泛化的 locale 字符串）。
 *
 * 结构：

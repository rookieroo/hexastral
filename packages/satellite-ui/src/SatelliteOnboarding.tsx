...
export async function hasCompletedSatelliteOnboarding(
  onboardingKey = DEFAULT_KEY
): Promise<boolean> {
  try {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      <Text style={[styles.kicker, { color: colors.secondary }]}>
        {props.copy?.kicker ?? 'HEXASTRAL SATELLITE'}
      </Text>
      ...
      {step === 'auth' ? (
        ...
        >
          <Text style={[styles.ctaText, { color: colors.text }]}>
            {props.copy?.getStarted ?? 'Get Started'}
          </Text>
        </Pressable>
      )}
    </View>

...
> The matrix is now a **monetization** split: only Kindred + Fēng are IAP-producing
> flagships; FaceOracle is a Tier-2 high-cost paid utility; everyone else is a Tier-3
> anonymous funnel. HexAstral (命Kindred卦道) is **retired** — the new Tier-3 `fate-app`
...
│   ├── Kindred / Kindred  — relationships (pairs · bonds · invites)
...
└── Tier 3 — anonymous ASO funnels, no IAP, each owns one keyword cluster + brand color
    ...
    └── Auspice (黄历)     — 黄历/择日 daily utility (ADR-0010, in build); 朱泥 earth-red
```
...
> [phase-k-plan.md §K.1.6](../phase-k-plan.md) and Auspice's (黄历 / 择日 / 万年历) in
...
Kindred's EN landing targets **"compatibility test" + "synastry"** as its primary
...
The flagship Kindred → relationships and Fēng → feng-shui CTAs remain on the

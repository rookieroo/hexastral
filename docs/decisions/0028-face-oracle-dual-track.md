...
**Amended — 2026-08-21 (deep/shallow meters):** Pro photo-slot main billing **retired**. Pro meters: **3 deep readings / UTC month** + **1 Face shallow brief / UTC day**. Consumable `faceoracle_reading` remains one full deep (3 photos · 5 chapters). First Pro seal is always deep (not gated by “deep next”). Deep-next toggle defaults **off**. Deep same-photo regen is discouraged; if used, consumes a deep read.  
...
**Amended — 2026-07-21 (SuggestedLoci Pass 0):** Before Pass 1, code ranks clear VLM keys (+ natal/dayun/palm-side weights) into **`SuggestedLoci` top ~20**. Pass 1 writes **16–20** deep readings from that shortlist (hard floors: face≥5, each palm≥5, CAUTION≥2 + one retry). Coordinates stay extract/client-only — Pass 0 does not invent (x,y).

## Context

The physiognomy satellite needed a high-COGS path (3× Vision + LLM + natal) that still supports retention via subscription surfaces and push. An earlier `face-oracle-app` scaffold accumulated kindred-rsync baggage; the client was reset as a greenfield app (folder `xingqi-app`, display **Syel**).

Matrix rule: satellites share **infra and packages**, not each other’s product HTTP. Yuun owns `/api/auspice/*` (anonymous `deviceId`, `auspice_pro`). Syel is portfolio + HMAC + `faceoracle_pro`. Kindred already follows the pattern (own make-if/monthly routes + shared libs).
...
3. **Dual IAP** (opaque server ids — product UI says Syel):
   - Consumable `faceoracle_reading` — floor **USD 9.99** — one sealed dense **five-chapter** deep reading (left + right palm + face required). No Timeline / What-if / Chat / Living FAB.
   - Subscription `faceoracle_pro_monthly` (**USD 14.99**) / `faceoracle_pro_annual` (**USD 99.99**) → entitlement `faceoracle_pro`:
     - **History / 档案** — reading snapshots + period briefs.
     - **Deep readings:** **3 / UTC month** (first seal always deep; later deep via settings “deep next” or explicit oneshot). Overflow → buy consumable.
     - **Face shallow briefs (`period_brief`):** **1 / UTC day** after the first seal (Face new photo; palms may reuse featureIds).
     ...
     - Same-photo **report regen** for deep is **not a primary product surface** (prefer new capture); if invoked, it consumes a **deep** monthly unit.
4. **API ownership (Life axis / What-if):**
   - **Do not** call `/api/auspice/timeline`, `/makeif*`, `/monthly` from Syel.
...
9. **UI tokens:** `CoreUIProvider brand='faceoracle'` (jade palette). Display strings say Syel.
...
- AGENTS birth-info table: Syel (`xingqi-app`) requires birth + three photos.

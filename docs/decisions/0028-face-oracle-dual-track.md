...
**Amended — 2026-07-20 (loci · palm sides · full-life · de-dup):** Still six chapters, with role tweaks: `natal` becomes the **全人生 timeline + 未来主章** (full 8-step 大运 ladder `dayunFull`/`dayunFuture` injected into `natalSummary`); `period` stays **近窗 only** and keeps the `events[]` push contract (≥2 near-window). Gender-based **掌别** (男 左=先天/右=后天; 女 右=先天/左=后天) is injected as `palmConvention`; palms chapter reads both hands. Coordinates come from a **Moondream 3.1 `point`** pass (schema `xingqi-vlm-v6`); low/empty coords warn, never hard-reject a clear photo. Density retry is **structural (no 字数 floors)**: field non-echo, cross-chapter distinctness (incl. `natal.dup_period`), citation/both-hands coverage, `natal.future_thin`, and event fuel. Reading `maxTokens` raised to 8192 (svc-feng parity) — no report batching.

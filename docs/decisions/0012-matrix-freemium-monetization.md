...
- Status: Accepted
- Date: 2026-05-21
- Amends: [ADR-0009](0009-two-layer-matrix.md) (Tier-3 is no longer "no IAP"; **Fēng & Face Oracle are NOT subscriptions**), [ADR-0006](0006-satellite-tiers.md) (tier→instrument decoupled), [ADR-0010](0010-cycle-satellite.md) (**fate & Kindred gain timeline/notification layers + subscriptions**)
...
> **Amended 2026-05-21** (see the plan doc for the final model): (1) **fate AND Kindred** both get a proactive **Timeline** (fate = 命运 流年/流月/大运; Kindred = 关系 流年/合盘变化) — engine `astro-core/timeline.ts` shipped for fate, Kindred reuses it. (2) **Chat is multi-layer** — context scales on two axes by tier: history span (时间长度) + token budget (文本长度): free=L1 short → consumable=limited → subscription=full L1–L4 → universe=cross-app. (3) Per-use **price bands**: Fēng = **high**, Face Oracle = **medium**, Coin Cast / Dream / Numerology = **low**. (4) The 3 **subscription flagships** are **fate · Kindred · Auspice** (a monetization label, orthogonal to brand prominence).
...
**Key separation:** *brand tier* (Kindred/Fēng are flagships — premium design + prominence) is
...
|---|---|---|
| **Recurring** (daily / proactive timeline + chat) | **Auspice** (daily 黄历) · **fate** (命运 timeline + 流年/流月/大运 预警) · **Kindred** (关系 timeline + 每日合盘运势) | **per-app subscription** |
...
value of fate and Kindred is a **proactive timeline with advance node notifications** — fate: 流年/流月/大运
转折 (push 半年/一月前); Kindred: 关系流年 / 纪念日 / 合盘变化. These are **sparse-periodic** (not daily, so
they don't cannibalize Auspice's daily cadence) and must be **built** — reuse astro-core (`dayun` / `jieqi`
/ `synastry`) + Auspice's `lib/push.ts` pattern.
...
`universe_pro` = (a) unlock all **subscription** features (Auspice daily + fate & Kindred timelines) +
...
Subscription entitlements only — `kindred_pro`, `fate_pro`, `auspice_pro`; `universe_pro` grants those +
...
- **Subscriptions** → entitlement: `yuan_pro_{m,a}`→`kindred_pro` · `fate_pro_{m,a}`→`fate_pro` · `cycle_pro_{m,a}`→`auspice_pro` · `universe_pro_{m,a}`→ all + allowance + cross-app chat
...
- fate & Kindred become genuine recurring-revenue products; the matrix's revenue spreads across subs + one-shots + packs (healthier than "flagships-only subs").

### Negative
- **fate & Kindred timeline/notification layers are NEW builds** (not yet implemented).

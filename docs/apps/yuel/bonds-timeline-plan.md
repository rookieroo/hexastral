# Bonds Timeline Plan — 本我中心的多关系时间轴 (Kindred 订阅护城河)
...
B-yuan.2 client ❌ never built) into the **ego-centric multi-bond** timeline that is Kindred's subscription moat:
...
- **D3 — 推送走本地通知，不走 server cron.** 复用 cycle-app 范式([push.ts](apps/auspice-app/lib/push.ts))：服务端回「未来推送时刻表」，客户端用 expo-notifications 排滚动窗口，开 app 重排。无 push token、无 cron。
...
**Next step**: **BT.5 — device/EAS batch.** yuan-app Timeline 屏 + 节点 explain sheet(打 `POST /api/bonds/timeline/explain` {bondId,year,nodeType,daYunOf})+ 本地推送(cycle `push.ts` 范式, 消费 `GET /api/bonds/timeline` 的 `notifications`)+ solo/resonance 录入表单收全精度(API 已接). 全部服务端契约就绪。**客户端类型契约也已预接** —— `packages/scenario-kindred` 加了 `BondsTimeline*` 类型 + `kindredBonds(client).timeline.{$get, explain.$post}` RPC → BT.5 = 纯 UI + hook + 推送排程 + 表单精度。

**✅ BT.5 客户端屏 + hook + 推送核心 SHIPPED (2026-06-05, `bun typecheck`+`bun test` green):**
- `apps/kindred-app/app/(timeline)/{_layout,index}.tsx` —— ego-centric 时间轴屏: 节点按年分组 + 显著度色点 + 点开 `explainNode` 内联深解 + 非 Pro upsell banner(→ `/(commerce)/paywall`)+ free 当前年 note. 入口从 Threads(bonds 列表)header pill 进。
- `packages/scenario-kindred`: `useBondsTimeline()` hook(`timeline.$get` + `timeline.explain.$post`, 服务端 `pro`/`upsell` 权威)+ `formatNodeSummary/Kind/LeadLabel`(客户端 4 语本地化 —— 引擎 summary 仅 zh-Hans)+ **纯 `buildTimelineNotificationPlan`**(未来过滤 + 按 key 去重 + 本地化 body, 7 vitest/bun-test 用例).
- **gate 复核**: `GET /api/bonds/timeline` 已用 `userHasCapability(...,'kindred')`(universe_pro 解锁全部)对非 Pro 清空 `notifications` —— D 的闸早已就位且正确, 无需改动.
- **仅剩真机一步**: on-device `expo-notifications` 排程封装(`scheduleTimelineNotifications` 调 `buildTimelineNotificationPlan`)—— 沙箱离线无法 `bun install` 该 dep(违 `--frozen-lockfile`), 留到 EAS/native build. 纯 builder 已测好, 封装 ≈30 行 expo-notifications 样板.
...
- 本地推送: 镜像 [cycle-app/lib/push.ts](apps/auspice-app/lib/push.ts) (`scheduleDailyAlmanac`/`refreshDailyPush` 范式) —— 从服务端 `notifications` 排滚动窗口，开 app 重排。全局限额已在服务端套好。
...
## 6. fate_pro / kindred_pro 边界

| 卖点 | 归属 | 引擎 |
|---|---|---|
| 本命 大运/流年 走势 (单人前瞻) | **fate_pro** (fate-app) | `getTimelineNodes` |
| 本我 × 全关系 的关系节点日历 | **kindred_pro** (yuan-app) | `composeBondsTimeline` (本计划) |

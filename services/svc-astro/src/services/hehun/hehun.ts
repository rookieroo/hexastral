...
const SYNASTRY_CHAPTER_SPECS: ReadonlyArray<{
...
}> = [
  {
    ...
    label: '第一印象',
    focus:
      '【本章主张】点名：最先吸引的一点 + 最先刺痛的一点（必须选边写清，禁止纯赞美）。日主与格局气场如何造成这两种反应。',
  },
  {
    kind: 'communication',
    label: '沟通方式',
    focus:
      '【本章主张】点名：谁更容易先打断/先冷战，以及「谁先改什么」。给出最有效沟通频道与最易翻车句式。勿复述第一印象气场描写；禁止「双方都要多理解」圆滑收束。',
  },
  {
    kind: 'conflict',
    label: '冲突源头',
    focus:
      '【本章主张】摩擦的真正根源（刑冲害/相克/格局张力）——主张「最容易被什么点燃、升级成什么」。本章以风险为主；禁止写成「其实是互补」；禁止两边都有道理。',
  },
  {
    kind: 'complement',
    label: '互补之处',
    focus:
      '【本章主张】分清谁补谁（甲方补乙方什么 / 乙方补甲方什么，不可对调）。并主张「互补失效时」会变成什么消耗。勿复述冲突章刑冲句。',
  },
  {
    kind: 'monthly_outlook',
    label: '本月相处参考',
    focus:
      '【本章主张】未来约30天：从关系场景库里选命盘能对上的 1–2 条（如深聊/推进订婚节奏/少逼问结婚/共同财务谈法/见家长边界），哪段宜推进、哪段宜留白——必须选边给窗口与动作。勿复述长期建议；禁止「整月都要注意沟通」；禁止场景清单倾泻。',
  },
  {
    kind: 'long_term_advice',
    label: '长期建议',
    focus:
      '【本章主张】各自必须软化的一个模式（直白点名）+ 共同经营规则清单。规则须落到能对上的具体场景（同居节奏/家人边界/共事信任/结婚窗口等择对上的写），禁止空话与「因人而异」；勿复述前五章金句。',
  },
]
...
## 写作要求（这一章要"值钱"：直白、详尽、有风险、可执行、不与他章复读）
分四层撰写，每层都**必须直接引用上方命盘事实块中的具体字符串**（日主干支/格局名/地支刑冲合/神煞/喜忌，以及紫微命宫/夫妻宫主星等），禁止泛泛而谈、禁止占星化通用语、禁止空洞鸡汤、禁止一味好听：
1. evidence（命盘依据，100–160字）：这一维度上双方命盘到底发生了什么——点名具体干支/十神/刑冲合/神煞，机理讲透；若八字与紫微同向，点出「两套系统不约而同」。禁止不引用事实的心理描写。
2. dynamic（关系动态，100–160字）：落到两人真实相处的可感画面（谁先炸、谁先撤、什么话题易翻车）。禁止「需要磨合」「互相理解」空壳；禁止与 evidence 同义复述。
3. reef（暗礁·风险，90–140字）：本章**必须有可点名的风险主张**——如何被触发、升级成什么、在什么相处节奏下更易发作。点到痛处；可用「宜留意」柔化语气，但内容必须选边，不许软成表扬。severity 按真实锋利度给 mid/high 时不要不敢给。
4. remedy（相处调整，100–160字）：围绕**用神**给出**选边**的明确指导，必须写清：
   - **做什么**（可执行步骤，最好可本周做）——写「做 A」，不要写「既可以 A 也可以 B」
   - **何时做或缓**（相对时间：接下来几周/冲突初起24小时内/这个月中段）
   - **避免什么**（冒进表态、翻旧账、冷处理过久等）——写「不要 B」
   禁「保持平衡」「多沟通」「给彼此空间」「一切都会好」「因人而异」「两边都有道理」等无主张空话。

另需：
- severity：reef 的严重度，必须是 low | mid | high 之一（英文小写）。无实质风险时仍至少 low，并写清「若忽略会怎样」。
- goldenLine：1–3句可截图金句——要有锋利主张，不要空心鸡汤；若点五行，须用双方日主真实五行关系，不可以用神冒充相生。
- title：≤14字，贴合本章。
- counterpoint（可选，≤60字）：转机注脚；不可用来把 reef 整段否定掉。
- 低分关系不说"不合适"，说"需要按以下方式磨合"并给出步骤；高分关系也必须提醒经营失败时的具体翻车点。
- **章内去重**：evidence / dynamic / reef / remedy 不得互相换皮复述；每层新信息。
- **章间去重**：只打本章 focus 主张；不要写他章会写的同一套「日主相生所以很配」套话。
- **不作户籍铁口**；关系预警与明确指导必须给足。
- **无主张的均衡废话视为失败输出**。
- **语体**：典雅克制的命理书面语。严禁网络话术（CP感/能量场/磁场/上头等）。宁古雅，勿轻佻；宁直白有主张，勿虚无圆滑。
...
export async function generateSynastryChapters(
  ...
  const systemPrompt = [
    ...
    '',
    '## 合盘正文原则',
    '- 每章必须落到双方命盘的具体特征上：直白现状 + 明确风险主张 + 选边的可执行指导',
    '- 禁止一味好听、禁止章间换皮复述、禁止空洞鸡汤',
    '- 无主张的均衡废话（「既可以…也可以…」「因人而异」「两边都有道理」「保持平衡即可」）视为失败输出',
    '- 结尾给能动性，但不要用「温暖收束」把风险说没',
    '- 正文中一律以「甲方」「乙方」称呼两人，不要使用或杜撰任何具体姓名（客户端会把称呼替换为「你」与对方名字）',
    '',
    '## 关系向场景库（匹配，勿清单倾泻）',
    '示例场景——按双方命盘张力选出真正能对上的 2–4 条贯穿六章着重讲，对不上的跳过；禁止整库硬塞。',
    '场景库：相亲/初见判断；恋爱推进 vs 宜缓；结婚/订婚节奏（窗口非「必将结婚」）；同居/共同生活节奏；',
    '亲密矛盾怎么谈；家人介入与边界；共事/合伙相处；远距离或迁移对关系的压力；共同财务节奏（非荐股）。',
    'monthly_outlook / long_term_advice 至少各点名 1 个能对上的具体场景并选边给窗口与动作。',
    buildLanguageBlock(language, 'hehun'),
  ].join('\n')
...
## 分析重点
聚焦 ${input.queryYear} 年太岁对甲乙双方日柱的影响，评估——须言之有物，禁止空洞套话：
1. 该年流年天干地支与双方四柱的互动参照（点名干支/刑冲合）
2. 该年双人感情/合作节奏的整体走向
3. 年内节奏分布：上半年/下半年各给「宜推进 / 宜维护 / 宜少冒进」之一
4. keyMonths：点名 2–3 个具体月份感 + 原因 + 双方可做的一步（谈事/缓推/抓共同机遇）
5. advice：可执行的相处或合作策略（沟通顺序、边界、共同项目节奏），禁止「多沟通」「保持耐心」无步骤句
6. 不作户籍铁口；要给关系预警与务实建议

## 输出要求（JSON 格式）
{
  "yearOverview": "${input.queryYear}年总体双人节律（150字，评估太岁对双方的整体影响，含命盘锚点）",
  "springFall": "上半年（春夏）节奏走势（80字，含宜推进或宜维护的提示）",
  "summerWinter": "下半年（秋冬）节奏走势（80字，含宜推进或宜缓的提示）",
  "keyMonths": "重要月份提示（80–100字，点名2-3个值得留意的月份、原因、双方可做的一步）",
  "joint": "双方协同建议（80–100字，如何共同把握${input.queryYear}年的窗口——具体动作）",
  "opportunities": "年度机遇亮点（60字，点名窗口类型）",
  "challenges": "年度挑战提示（60字，点名触发条件与缓冲动作）",
  "advice": "应对策略（80–100字，具体行动建议：谈/缓/抓机遇）",
  "summary": "一句话年度总结（15字以内，含年号或太岁感）"
}

...
## 写作要求（这一章要"值钱"：详尽、精准、可执行、有关键节点）
分四层撰写，每层都**必须直接引用上方命盘事实**（具体日主/格局/地支刑冲合/神煞/五行喜忌，以及紫微命盘要点：命宫/夫妻宫主星、飞星落宫），禁止泛泛而谈、禁止占星化通用语、禁止空洞鸡汤：
1. evidence（命盘依据，90–150字）：这一维度上双方命盘到底发生了什么——点名具体的干支/十神/刑冲合/神煞，把机理讲清楚；若八字与紫微（上方第二套系统）指向一致，明确点出「两套系统不约而同」以增强可信度。
2. dynamic（关系动态，90–150字）：上述命理如何落到两人**真实相处**的样子，具体可感（沟通节奏、冲突触发、亲密距离）——禁止只写「需要磨合」「互相理解」。
3. reef（暗礁·风险，70–130字）：本章最该警惕的**一个**具体风险/危机——如何被触发、会演变成什么、**大约在什么相处节奏/季节感下更易发作**。点到痛处，但不下"天命定论"。
4. remedy（相处调整，90–150字）：围绕**用神**给出具体、可执行的调整/增进之道。必须写清：
   - **做什么**（谈边界 / 先听后辩 / 少同时推两件大事 / 何时适合深谈）
   - **何时做或缓**（「接下来几周」「这个月中段」「冲突刚起的 24 小时内」等相对时间——勿写死会过期的公历年月日）
   - **避免什么**（冒进表态、翻旧账、冷处理过久等）
   落到能动的一面（观照节奏、自我努力）。禁「保持平衡」「多沟通」「给彼此空间」等无步骤空话。

另需：
- severity：reef 的严重度，必须是 low | mid | high 之一（英文小写）。
- goldenLine：1–3句可截图分享的金句，精炼、有画面感、点中本章本质；须含命盘或相处机制锚点，勿空心鸡汤；若点到五行，须用双方日主真实的两个五行及其生/克/比和，不可以用神之五行冒充双方的相生关系。
- title：≤14字，贴合本章。
- counterpoint（可选，≤60字）：一句留有转机/希望的注脚（如时柱相合、流月转气），没有可省略或留空。
- 低分关系不说"不合适"，而说"需要更多理解与磨合"并给出磨合**怎么做**；高分关系也要提醒"好的关系仍需经营"并给出**经营节点**。
- **不作户籍铁口**（是否结婚、子女人数、家人性格档案），但**必须**给关系预警与务实建议（矛盾怎么谈、何时维护、何时少逼问）。

...
const SYNASTRY_CHAPTER_SPECS: ReadonlyArray<{
  ...
  },
  {
    ...
    label: '长期建议',
    focus:
      '【本章主张】各自必须软化的一个模式（直白点名）+ 共同经营规则清单。必须含约十年级/多阶段节奏：哪段宜推进承诺或共同项目、哪段宜守边界（选边）。规则落到能对上的具体场景（同居/家人/共事/结婚窗口等）。禁止空话、「因人而异」、只写近期注意；勿复述前五章金句。真话优先于好话。',
  },
]
...
export async function generateSynastryChapters(
  ...
  const systemPrompt = [
    ...
    '',
    '## 合盘正文原则',
    '- 落到双方命盘：直白现状、风险主张、可执行指导；写完整段落。',
    '- 甲方/乙方称呼；真话优先；aha 要具体到日主/宫星张力。',
    buildLanguageBlock(language, 'hehun'),
  ].join('\n')
